...
class PostgresDdlGeneratorTableTest : FunSpec({
    ...
    test("biginteger identity with legacy serial syntax generates BIGSERIAL") {
        val s = schema(
            tables = mapOf(
                "orders" to table(
                    columns = mapOf(
        ...
        ddl shouldNotContain "\"id\" BIGSERIAL NOT NULL"
        ddl shouldNotContain "\"id\" BIGINT"
    }

    test("biginteger identity without legacy serial syntax generates explicit identity") {
        val s = schema(
            tables = mapOf(
                "events" to table(
                    columns = mapOf(
        ...
        val ddl = generator.generate(s).render()
        ddl shouldContain "\"id\" BIGINT GENERATED ALWAYS AS IDENTITY"
        ddl shouldNotContain "\"id\" BIGSERIAL"
    }

    test("integer identity without legacy serial syntax generates explicit identity") {
        val s = schema(
            tables = mapOf(
                "jobs" to table(
                    columns = mapOf(
        ...
        ddl shouldNotContain "\"id\" SERIAL"
    }

    test("plain biginteger remains BIGINT") {
        val s = schema(
            tables = mapOf(
        ...
        ddl shouldNotContain "\"value\" BIGSERIAL"
        ddl shouldNotContain "\"value\" BIGINT GENERATED"
    }

    test("owned identity sequence is not generated as standalone sequence") {
        val s = schema(
            sequences = mapOf(
                "orders_id_seq" to SequenceDefinition(start = 1, increment = 1),
                "invoice_seq" to SequenceDefinition(start = 1000, increment = 1),
            ),
            tables = mapOf(
                "orders" to table(
                    columns = mapOf(
