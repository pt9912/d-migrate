...
/**
 * Plan-2 §F.4 second slice: PostgreSQL renderer pins
 * `ALTER TABLE … RENAME TO …` and
 * `ALTER TABLE … RENAME COLUMN … TO …` for both Up and Down
 * directions.
 */
class PostgresDiffRenameTest : FunSpec({
    ...
    test("table rename Up renders ALTER TABLE … RENAME TO …") {
        val current = emptySchema().copy(tables = mapOf("users_old" to table))
        val desired = emptySchema().copy(tables = mapOf("users" to table))
