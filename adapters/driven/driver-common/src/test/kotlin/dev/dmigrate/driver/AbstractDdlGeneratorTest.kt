...
class AbstractDdlGeneratorTest : FunSpec({
    ...
    test("generate() includes the standard header as first statement") {
        ...
        header shouldContain "Generated by d-migrate ${VersionInfo.PRODUCT_VERSION}"
        header shouldContain "Source: neutral schema v1.0.0"
        header shouldContain "Target: postgresql"
    }

    // ddl-comment-rendering.md: ddl.include_comments = false unterdrueckt den
    // Header, ohne die uebrigen Statements zu beruehren.
    test("generate() omits the header when includeComments is false") {
        val gen = TestDdlGenerator()
        val schema = schema("t" to table())
        val result = gen.generate(schema, DdlGenerationOptions(includeComments = false))
        result.statements.none { it.sql.contains("Generated by d-migrate") } shouldBe true
        result.statements.first().sql shouldContain "CREATE TABLE"
    }

    test("generate() keeps the header when includeComments is true (explicit)") {
        val gen = TestDdlGenerator()
        val schema = schema("t" to table())
        val result = gen.generate(schema, DdlGenerationOptions(includeComments = true))
        result.statements.first().sql shouldContain "Generated by d-migrate"
    }

    // Silent-Fallback-Guard: schützt den `Generated by`-Header davor, sich auf
    // die `unknown`-Fallback-Variante einzulassen, wenn :hexagon:core's
    // dmigrate-version.properties hier nicht auf dem Classpath landet.
    // Ohne diesen Guard würden die obigen `shouldContain`-Assertions
    // sich selbst „bestätigen" (beide Seiten interpolieren denselben
    // Singleton), und ein Resource-Klassenpfad-Bruch bliebe in sharded
    // CI-Runs unsichtbar.
    test("VersionInfo.PRODUCT_VERSION resolves on the driver-common classpath") {
        VersionInfo.PRODUCT_VERSION shouldNotBe "unknown"
    }

    test("generate() uses generatedAt option for the header timestamp") {
        val gen = TestDdlGenerator()
        val fixed = Instant.parse("2026-04-01T00:00:00Z")
        val result = gen.generate(schema("t" to table()), DdlGenerationOptions(generatedAt = fixed))

        result.statements.first().sql shouldContain "Generated: 2026-04-01T00:00:00Z"
    }

    test("generate() omits the header timestamp in deterministic mode") {
        val gen = TestDdlGenerator()
        val result = gen.generate(schema("t" to table()), DdlGenerationOptions(deterministic = true))

        result.statements.first().sql shouldContain "Target: postgresql"
        result.statements.first().sql shouldNotContain "Generated:"
    }
