...
internal object OracleDiffTableOps {
    ...
    /** Zwischen Identity und Berechnung wechseln -- kein `MODIFY` deckt das. */
    const val IDENTITY_COMPUTED_SWAP_NOT_SUPPORTED: String = "ORACLE_IDENTITY_COMPUTED_SWAP_NOT_SUPPORTED"
    ...
    fun renderAlterColumnGeneration(op: DiffOperation.AlterColumnGeneration, ctx: OracleDiffRenderContext) {
        ...
        val expressions = setOfNotNull(
            (from as? ColumnGeneration.Computed)?.expression,
            (target as? ColumnGeneration.Computed)?.expression,
        )
    ...
    /**
     * Die Identity einer Spalte, soweit Oracle sie in place aendert -- gemessen
     * gegen 23:
     *
     * | Uebergang | Ergebnis |
     * | --- | --- |
     * | `ALWAYS` ↔ `BY DEFAULT` | `MODIFY (c GENERATED … AS IDENTITY)`, der Zaehler behaelt seinen Stand |
     * | Identity → gewoehnlich | `MODIFY (c DROP IDENTITY)`, Werte bleiben, die Sequenz verschwindet |
     * | gewoehnlich → Identity | **`ORA-30673`** -- „column to be modified is not an identity column" |
     * | Identity ↔ berechnet | kein `MODIFY` deckt das ab |
     */
    private fun renderIdentityTransition(
        ...
        when {
            fromIdentity != null && targetIdentity != null -> ctx.emit(
                op,
                "ALTER TABLE ${ctx.sql.quote(table)} MODIFY ${ctx.sql.quote(column)} " +
                    "GENERATED ${identityWord(targetIdentity)} AS IDENTITY;",
            )

            fromIdentity != null && target == null -> {
                ctx.emit(
                    op,
                    "ALTER TABLE ${ctx.sql.quote(table)} MODIFY ${ctx.sql.quote(column)} DROP IDENTITY;",
                )
                // Live gemessen: Oracle nimmt der Spalte mit der Identity auch
                // ihr implizites NOT NULL (`required` kippte von true auf
                // false, der Post-Compare meldete danach Drift). Verlangt das
                // Soll die Spalte weiter als Pflichtfeld, muss der Zwang
                // nachgezogen werden -- zwei Anweisungen fuer eine Operation
                // sind vertraglich zulaessig.
                val stillRequired = ctx.schemaForDirection()
                    ?.tables?.get(table)?.columns?.get(column)?.required == true
                if (stillRequired) {
                    ctx.emit(
                        op,
                        "ALTER TABLE ${ctx.sql.quote(table)} MODIFY ${ctx.sql.quote(column)} NOT NULL;",
                    )
                }
            }
            ...
            targetIdentity != null -> {
                ctx.skip(
                    op,
                    "Operation ${op.id} would make '$table.$column' an identity column. Oracle refuses that " +
                        "with ORA-30673 (\"column to be modified is not an identity column\", measured " +
                        "against 23); the column has to be recreated, or expressed through the column type " +
                        "(`identifier`).",
                    code = IDENTITY_ADD_NOT_SUPPORTED,
                )
                ctx.addBlocker(MigrationBlockedReason.MANUAL_ACTION_REQUIRED, operationIds = setOf(op.id))
            }
                ...
                ctx.skip(
                    op,
                    "Operation ${op.id} swaps identity and computation on '$table.$column'. Oracle has no " +
                        "`MODIFY` that covers it -- a real column cannot take an expression (ORA-54026), and " +
                        "an identity column is not one either; recreate the column manually.",
