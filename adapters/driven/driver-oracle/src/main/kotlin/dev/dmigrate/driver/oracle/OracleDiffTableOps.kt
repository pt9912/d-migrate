...
internal object OracleDiffTableOps {
    ...
    /**
     * Aus einer berechneten Spalte eine gewoehnliche machen. Oracle **kann**
     * das (`MODIFY (c <typ>)`, gemessen an 23) -- d-migrate rendert es nicht.
     */
    ...
    /** Die Identity einer Spalte aendern -- kein Ausdrucksfall. */
    const val IDENTITY_CHANGE_NOT_SUPPORTED: String = "ORACLE_IDENTITY_CHANGE_NOT_SUPPORTED"
    ...
    fun renderAlterColumnGeneration(op: DiffOperation.AlterColumnGeneration, ctx: OracleDiffRenderContext) {
        ...
        when (ColumnGenerationTransition.of(from, target)) {
            ColumnGenerationTransition.IDENTITY -> {
                ctx.skip(
                    ...
                    "Operation ${op.id} changes the identity of '$table.$column' -- not a computed " +
                        "expression. Oracle does two of these in place (`MODIFY (c GENERATED BY DEFAULT AS " +
                        "IDENTITY)` and `MODIFY (c DROP IDENTITY)`, both measured against 23), while making " +
                        "an ordinary column an identity one fails with ORA-30673. d-migrate does not render " +
                        "identity changes: express the transition through the column type (`identifier`) or " +
                        "run the statement yourself.",
            ...
            ColumnGenerationTransition.COMPUTED_DROPPED -> {
                // Hier stand, Oracle habe kein `MODIFY`, das eine berechnete
                // Spalte zurueckverwandelt. Nachgemessen an 23 ist das falsch:
                // `ALTER TABLE "t" MODIFY ("g" NUMBER)` laeuft durch. Gerendert
                // wird es trotzdem nicht -- der Uebergang ist ungebaut, und
                // eine Zusage ohne Abnahme waere dasselbe Problem von der
                // anderen Seite.
                ctx.skip(
                    op,
                    "Operation ${op.id} would drop the computed expression of '$table.$column'. Oracle " +
                        "accepts `MODIFY ($column <type>)` for a VIRTUAL column (measured against 23), but " +
                        "d-migrate does not render that transition -- run it yourself, or drop and recreate " +
                        "the column.",
