...
internal class MysqlSequenceSupport {
    ...
    fun scanSequenceSupport(
        ...
        if (exists == null) return MysqlSequenceSupportSnapshot.nonSequence(scope).copy(supportTableState = SupportTableState.NOT_ACCESSIBLE)
        ...
        val rawRows = try { MysqlMetadataQueries.listSupportSequenceRows(session, database) } catch (_: Exception) {
            return earlySnapshot(scope, SupportTableState.NOT_ACCESSIBLE, "dmg_sequences exists but rows are not readable")
        }
        ...
        return MysqlSequenceSupportSnapshot(scope = scope, supportTableState = SupportTableState.AVAILABLE,
            sequenceRows = sequenceRows, ambiguousKeys = ambiguousKeys, routineStates = routineStates,
            triggerStates = triggerScan.triggers.associate { it.triggerName to it.state },
            triggerAssessments = triggerScan.triggers,
            diagnostics = buildSupportDiagnostics(scope, sequenceRows, ambiguousKeys, routineStates,
                triggerScan.triggers, triggerScanAccessible = triggerScan.accessible, confirmedSequenceNames = dmigrateRowNames))
    }

    private fun verifySupportTableShape(session: JdbcOperations, database: String, scope: ReverseScope): MysqlSequenceSupportSnapshot? {
        val shapeOk = try { MysqlMetadataQueries.checkSupportTableShape(session, database) } catch (_: Exception) {
            return earlySnapshot(scope, SupportTableState.NOT_ACCESSIBLE, "dmg_sequences exists but metadata is not accessible")
        }
        if (!shapeOk) return earlySnapshot(scope, SupportTableState.INVALID_SHAPE, "dmg_sequences column shape is not canonical")
        return null
    }

    private fun earlySnapshot(scope: ReverseScope, state: SupportTableState, cause: String) = MysqlSequenceSupportSnapshot(
        scope = scope, supportTableState = state, sequenceRows = emptyList(), ambiguousKeys = emptySet(),
        routineStates = emptyMap(), triggerStates = emptyMap(), triggerAssessments = emptyList(),
        diagnostics = listOf(SupportDiagnostic(key = SequenceDiagnosticKey(scope, MysqlSequenceNaming.SUPPORT_TABLE), causes = listOf(cause), emitsW116 = true)))

    private fun mapSequenceRow(row: Map<String, Any?>): SequenceRowSnapshot {
        val rawMin = row["min_value"]; val rawMax = row["max_value"]; val rawCache = row["cache_size"]
        return SequenceRowSnapshot(name = row["name"] as? String, nextValue = safeLong(row["next_value"]),
            incrementBy = safeLong(row["increment_by"]), minValue = safeLong(rawMin), maxValue = safeLong(rawMax),
            cycleEnabledRaw = safeCycleInt(row["cycle_enabled"]), cacheSize = safeInt(rawCache),
            managedBy = row["managed_by"] as? String, formatVersion = row["format_version"] as? String,
    ...
    private fun detectAmbiguousKeys(rows: List<SequenceRowSnapshot>): Pair<Set<String>, Set<String>> {
        fun isDMigrate(r: SequenceRowSnapshot) = r.managedBy?.trim() == "d-migrate" && r.formatVersion?.trim() == "mysql-sequence-v1"
        val names = rows.filter { isDMigrate(it) && !it.name?.trim().isNullOrEmpty() }.map { it.name!!.trim() }
        val ambiguous = names.groupBy { it }.filter { it.value.size > 1 }.keys
        return ambiguous to names.filter { it !in ambiguous }.toSet()
    }

    private fun scanRoutineStates(session: JdbcOperations, database: String) = mapOf(
        MysqlSequenceNaming.NEXTVAL_ROUTINE to MysqlMetadataQueries.lookupSupportRoutine(session, database, MysqlSequenceNaming.NEXTVAL_ROUTINE),
        MysqlSequenceNaming.SETVAL_ROUTINE to MysqlMetadataQueries.lookupSupportRoutine(session, database, MysqlSequenceNaming.SETVAL_ROUTINE))
    ...
    fun materializeSupportSequences(
        ...
        for (row in snapshot.sequenceRows) {
            val managedBy = row.managedBy?.trim() ?: continue
            val formatVersion = row.formatVersion?.trim() ?: continue
            if (managedBy != "d-migrate" || formatVersion != "mysql-sequence-v1") continue
            ...
            if (name in snapshot.ambiguousKeys) continue
    ...
    private fun buildSupportDiagnostics(
        scope: ReverseScope,
        _rows: List<SequenceRowSnapshot>,
        ambiguousKeys: Set<String>,
        _routineStates: Map<String, SupportRoutineState>,
