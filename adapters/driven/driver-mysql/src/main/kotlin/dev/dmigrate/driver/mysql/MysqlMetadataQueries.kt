...
object MysqlMetadataQueries {
    ...
    fun checkSupportTableShape(session: JdbcOperations, schemaName: String): Boolean =
        MysqlSequenceSupportMetadataQueries.checkSupportTableShape(session, schemaName)

    fun listSupportSequenceRows(session: JdbcOperations, schemaName: String): List<Map<String, Any?>> =
        MysqlSequenceSupportMetadataQueries.listSupportSequenceRows(session, schemaName)
    ...
    fun lookupSupportRoutine(
        ...
        MysqlSequenceSupportMetadataQueries.lookupSupportRoutine(session, schemaName, routineName)
