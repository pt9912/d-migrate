...
class SchemaGenerateRunnerErrorTest : FunSpec({
    ...
    class FakeGenerator(
        ...
        val lastOptions: dev.dmigrate.driver.DdlGenerationOptions?
            get() = rollbackOptions ?: generateOptions
    ...
    fun request(
        ...
        report = report, generateRollback = generateRollback,
    ...
    test("Exit 0: same options passed to generate and generateRollback") {
        val h = harness()
        h.runner().execute(
            request(target = "postgresql", spatialProfile = "none", generateRollback = true)
        ) shouldBe 0
    ...
    test("--split pre-post without --output and without json exits 2") {
        ...
        exit shouldBe 2
        h.stderr.joined() shouldContain "`--split pre-post` requires `--output`"
    }
    ...
    test("JSON output includes generator version 0.9.6.1") {
        val h = harness()
        h.runner().execute(request(outputFormat = "json")) shouldBe 0
        h.stdout.joined() shouldContain "\"generator\": \"d-migrate 0.9.6.1\""
    }
})
