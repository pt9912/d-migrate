...
class CliGenerateTestPart2 : FunSpec({
    ...
    test("E2E: DDL header contains d-migrate 0.9.5 version") {
        val source = writeTempSchema(minimalSchemaYaml)
        try {
            ...
            out shouldContain "Generated by d-migrate 0.9.5"
        } finally {
            Files.deleteIfExists(source)
        }
    }

    test("E2E: Report contains d-migrate 0.9.5 generator string") {
        val source = writeTempSchema(minimalSchemaYaml)
    ...
    test("E2E: --output-format json with --target mysql includes generator 0.9.5") {
        val source = writeTempSchema(minimalSchemaYaml)
        try {
            val (out, _) = captureStreams {
                shouldNotThrowAny {
            ...
            out shouldContain "\"generator\": \"d-migrate 0.9.5\""
        } finally {
            Files.deleteIfExists(source)
        }
    }
