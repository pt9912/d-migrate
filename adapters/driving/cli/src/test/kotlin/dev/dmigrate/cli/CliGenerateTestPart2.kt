...
class CliGenerateTestPart2 : FunSpec({
    ...
    test("E2E: DDL header contains the current d-migrate version") {
        val source = writeTempSchema(minimalSchemaYaml)
        try {
            ...
            out shouldContain "Generated by d-migrate ${VersionInfo.PRODUCT_VERSION}"
        } finally {
            Files.deleteIfExists(source)
        }
    }

    test("E2E: Report contains the current d-migrate generator string") {
        val source = writeTempSchema(minimalSchemaYaml)
    ...
    test("E2E: --output-format json with --target mysql includes the current generator version") {
        val source = writeTempSchema(minimalSchemaYaml)
        try {
            val (out, _) = captureStreams {
                shouldNotThrowAny {
            ...
            out shouldContain "\"generator\": \"d-migrate ${VersionInfo.PRODUCT_VERSION}\""
        } finally {
            Files.deleteIfExists(source)
        }
    }
