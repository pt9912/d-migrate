...
| W134 | Migrate: `enum`-Spalte als bloßes `TEXT` migriert, Werte nicht durchgesetzt — die Spalte führt weder eigene Werte (die ein `CHECK` durchsetzt) noch einen `ref_type` (den ein eigener Typ trägt) |
