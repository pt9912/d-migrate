...
die ueber REST (§4.4 in `spec/rest-service.md`) oder MCP bedient werden.
...
`ArtifactMetadata` enthaelt die Pflichtfelder aus `spec/job-contract.md` §3.1
...
- alle RPCs, die `JobAccepted` zurueckgeben (`ReverseSchema`, `CompareSchema`,
  ...
  wird mit `ALREADY_EXISTS` abgewiesen (siehe `spec/job-contract.md` §5)
