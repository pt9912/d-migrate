...
reverse:
  sqlite:
    ...
    # Ob ein unter Breite 64 als Identity-Spalte gelesener AUTOINCREMENT-
    # Schluessel als `serial` (Default, legacy_serial_syntax) oder als
    # `identity` (ohne das Flag) gemeint ist; unter Breite 32 ohne Wirkung.
    # Prioritaet: CLI-Flag `--sqlite-autoincrement-syntax` > Config-Wert > Default.
    autoincrement_syntax: serial
  mysql:
    # Wie der MySQL-Reverse ein `BIGINT AUTO_INCREMENT` schreibt:
    # serial → generation: identity mit legacy_serial_syntax (Default,
    # PostgreSQL erzeugt BIGSERIAL) · identity → ohne das Flag (PostgreSQL
    # erzeugt GENERATED BY DEFAULT AS IDENTITY).
    # Prioritaet: CLI-Flag `--mysql-autoincrement-syntax` > Config-Wert > Default.
    autoincrement_syntax: serial
