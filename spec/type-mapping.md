...
| `smallint` `GENERATED ALWAYS AS IDENTITY` im Primärschlüssel | `identifier` mit `auto_increment` | Der Modus geht verloren, und zwar ohne Code: das neutrale Modell trägt eine Identity nur auf `integer` und `biginteger` (`E130`), und kein Generator rendert eine `smallint`-Identity. Als `smallint` + `identity` gelesen ließe sich die Datei nicht mehr erzeugen |
| `integer`/`smallint` `GENERATED BY DEFAULT AS IDENTITY` bzw. `serial` im Primärschlüssel | `identifier` mit `auto_increment` | Der 32-Bit-Autowert-Vertrag; hier verspricht das Modell nichts, was das Rendern bricht |
