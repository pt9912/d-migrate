...
| ein Feld eines zusammengesetzten Typs | wie die gleichnamige Spaltenregel | dieselben Codes an derselben Stelle: `R404` für `numeric` ohne Präzision, `R301` für einen unbekannten Feldtyp, `R402` für ein `json`-Feld (`CREATE TYPE … AS (…)` schreibt den Feldtyp durch denselben Mapper wie eine Spalte, also als `jsonb`) |
...
| `integer` `GENERATED ALWAYS AS IDENTITY` im Primärschlüssel | `integer` + `generation: identity` (Modus `always`) | Der Typ `identifier` trägt keinen Modus und rendert als `SERIAL`, das gesetzte Werte annimmt; `always` wäre damit schon PostgreSQL → PostgreSQL verloren, ohne dass ein Vergleich zweier Reverses es sieht. Es zählt die Zugehörigkeit zum Schlüssel, nicht seine Spaltenzahl: ein Mitglied eines mehrspaltigen Schlüssels liest genauso |
| `smallint` `GENERATED ALWAYS AS IDENTITY` | `identifier` mit `auto_increment`, `R406` | Das neutrale Modell trägt eine Identity nur auf `integer` und `biginteger` (`E130`), und kein Generator rendert eine `smallint`-Identity. Als `smallint` + `identity` gelesen ließe sich die Datei nicht mehr erzeugen. Der Modus geht deshalb verloren — `identifier` rendert als `SERIAL`, und das nimmt gesetzte Werte an. `R406` benennt ihn. Es gilt die Breite, nicht die Schlüsselzugehörigkeit: dieselbe Spalte ohne Schlüssel liest genauso |
| `integer`/`smallint` `GENERATED BY DEFAULT AS IDENTITY` bzw. `serial` im Primärschlüssel | `identifier` mit `auto_increment` | Der 32-Bit-Autowert-Vertrag; hier verspricht das Modell nichts, was das Rendern bricht. Bei `smallint` mit `BY DEFAULT` ist `SERIAL` die richtige Entsprechung und meldet deshalb nichts; ein `smallserial` ist gar keine Identity |
| PostGIS-Geometrie, `geometry_columns` **nicht erreichbar** | `geometry` ohne Subtyp und ohne SRID, `R405` | Der häufige Grund ist ein PostGIS-Schema außerhalb des `search_path`. Die Note nennt ihn und den Ausweg (das Schema in den `search_path` aufnehmen oder Subtyp und SRID in der Schemadatei deklarieren); `R401` bleibt daneben als Spaltenhinweis. Eine Tabelle ohne Geometriespalte meldet nichts |
...
|---|---|---|
| `int` IDENTITY | `integer` + `generation: identity` (`ALWAYS`) | Dieselbe Lesart wie `bigint`: T-SQL kennt keinen `BY DEFAULT`-Modus, und `identifier` trägt **keinen** Modus — der Verlust fiel erst beim Rundweg über einen anderen Dialekt auf (eine PostgreSQL-Identity reist als `INT IDENTITY(1,1)` herüber und kam ohne Modus zurück) |
...
| `SDO_GEOMETRY` | `geometry` ohne Subtyp | SRID aus `ALL_SDO_GEOM_METADATA`, sofern eine Zeile mit exakt passendem Tabellen- und Spaltennamen existiert **und** einen SRID nennt; der Abgleich ist wortgetreu. Sonst ist die Spalte `geometry` **ohne** SRID und der Verlust wird gemeldet: `R370` (`WARNING`), wenn die Sicht lesbar ist — mit je eigenem Ausweg, ob die Zeile ohne SRID dasteht, ob es sie zu diesem Namen gar nicht geben kann oder ob sie nur fehlt —, `R365` (`WARNING`) an einer Tabelle mit Geometriespalte, wenn die Sicht nicht lesbar ist. Die beiden schließen sich aus, und beide blocken nichts |
