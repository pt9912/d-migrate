...
| `integer`/`smallint` `GENERATED ALWAYS AS IDENTITY` als **alleiniger** Primärschlüssel | Basistyp + `generation: identity` (Modus `always`) | Der Typ `identifier` trägt keinen Modus und rendert als `SERIAL`, das gesetzte Werte annimmt; `always` wäre damit schon PostgreSQL → PostgreSQL verloren, ohne dass ein Vergleich zweier Reverses es sieht |
| `integer`/`smallint` `GENERATED BY DEFAULT AS IDENTITY` bzw. `serial` als alleiniger Primärschlüssel | `identifier` mit `auto_increment` | Der 32-Bit-Autowert-Vertrag; hier verspricht das Modell nichts, was das Rendern bricht |
