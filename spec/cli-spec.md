...
| W163 | An identity column declared `mode: always` is rendered as an auto-increment column that accepts explicit values (MySQL, SQLite): the mode is not enforced in the target |
...
| `--inline-foreign-keys` | Nein | `auto` / `always` / `never` | Fremdschluessel-Platzierung (Default `auto`, folgt weiterhin `--split`: inline bei `single`, zurueckgestellt als `ALTER TABLE` nach allen Tabellen bei `pre-post`). `always` erzwingt Inline, unabhaengig von `--split`; `never` erzwingt Zurueckstellung, unabhaengig von `--split`. `never` auf einem Dialekt ohne Deferred-Constraint-Form (MySQL, SQLite): Exit 2. Praezedenz **CLI-explizit > `ddl.inline_foreign_keys` (Config) > `auto`**. |
...
**`legacy_serial_syntax` einer Identity-Spalte** bleibt ein Unterschied, in
jeder Paarung. Ob ein `BIGINT AUTO_INCREMENT` (MySQL) oder ein unter der
Breite `64` gelesener `AUTOINCREMENT`-Schluessel (SQLite) als `SERIAL` oder als
IDENTITY gemeint war, traegt die Datenbank nicht; das entscheidet die
Reverse-Praeferenz `serial`/`identity` (`--mysql-autoincrement-syntax`,
`--sqlite-autoincrement-syntax` bei `schema reverse`, sonst
`reverse.<dialekt>.autoincrement_syntax`, die auch die `db:`-Operanden von
`schema compare` lesen), nicht der Vergleich. Ohne Deklaration meldet eine
PostgreSQL-IDENTITY-Spalte gegen ein solches Reverse deshalb einen
Unterschied in `legacy_serial_syntax`; mit `identity` entfaellt er.

**Grenze — der Identity-Modus gegen einen Dialekt, der nur eine Form kennt:**
SQL Server kennt kein `GENERATED BY DEFAULT`. `schema generate` rendert eine
solche Spalte als `IDENTITY(1,1)` und meldet das mit `W140`; der Reverse liest
sie als `mode: always`. MySQL und SQLite kennen nur ihre Autowert-Form, die
Werte auch von Hand annimmt; `schema generate` und `schema migrate` melden eine
Spalte mit `mode: always` dort mit `W163`, und ihr Reverse liest jede solche
Spalte, die er als Identity-Spalte schreibt (SQLite unter der Breite `64`), als
`mode: by_default`, auch wenn das Soll `always` sagte. Der Modus bleibt in
beiden Richtungen ein Unterschied — ein Faehigkeitsunterschied, den
`schema compare` nicht wegdefiniert und den keine Reverse-Praeferenz aufloest.
...
| `--target-version` | Nein | Versionstext | Wie bei `schema generate`. **Gewinnt ueber die Version, die der Lesepfad meldet**: die Angabe sagt, wofuer das Skript gedacht ist, nicht, woraus gelesen wurde. |
| `--pg-concurrent-indexes` | Nein | Boolean | PostgreSQL: Indizes werden mit `CONCURRENTLY` angelegt und abgebaut, mit `transactionScope = NO_TRANSACTION` und `TransactionBehavior.NOT_TRANSACTIONAL`. Eine Aussage darueber, WIE migriert wird — deshalb eine Option des Laufs und kein Feld am Index; sie gilt fuer den ganzen Lauf und fuer beide Richtungen. Vor jedem nebenlaeufigen `CREATE` steht ein `DROP INDEX CONCURRENTLY IF EXISTS` desselben Namens: ein abgebrochener Lauf hinterlaesst einen Index mit `pg_index.indisvalid = false`, an dem der naechste Lauf sonst scheiterte. Jede so gerenderte Operation traegt eine INFO-Diagnose `POSTGRES_INDEX_CONCURRENTLY`. Ohne das Flag bleibt es beim gewoehnlichen `CREATE INDEX` innerhalb der Transaktion. |
| `--spatial-profile` | Nein | String | Wie bei `schema generate`, mit denselben Werten je Dialekt. Unbekannter Name oder ein Profil, das der Dialekt nicht fuehrt: Exit `2` vor jedem Datenbankzugriff. Mit `none` blockt ein Plan, der Geometrie einfuehrt (`CreateTable` mit Geometriespalte, `AddColumn`, `AlterColumnType` **nach** Geometrie), vollstaendig mit `E052` und `primaryBlockedReason = MANUAL_ACTION_REQUIRED` (Exit `8`) — nicht nur die betroffene Tabelle: ein Migrationsplan ist abhaengigkeitssortiert, eine Tabelle daraus zu entfernen liesse die uebrigen Anweisungen auf etwas verweisen, das nicht entsteht. **Mit `spatialite`** rendert der Migrate-Pfad `SELECT AddGeometryColumn(…)` und blockt nach derselben Ausloeserliste wie `schema generate` ([DDL-Generierungsregeln](./ddl-generation-rules.md), Spatial, SQLite): `unique`, ein `default`, ein Fremdschluessel an der Spalte, die Spalte im Primaerschluessel und eine tabellenweite Einschraenkung auf ihr. `required` blockt dort **nicht** mehr (die Metadaten-Funktion traegt die Nullbarkeit selbst) — ausser bei `ADD COLUMN` auf eine bestehende Tabelle und bei jedem Tabellen-Neubau, der eine Geometriespalte beruehrt. Auch diese Faelle blocken den ganzen Plan. |
...
### Overlay-Art `raw-text-provenance`

Traegt die **Herkunft** der vier Felder, die rohen SQL-Text fuehren: welcher
Autorentext zuletzt angewandt wurde, und welche Katalogform der Server
daraufhin fuehrt.

Der Grund ist, dass die beiden nie uebereinstimmen und die eine aus der anderen
nicht rekonstruierbar ist: der Server druckt den Ausdruck aus seinem Parsebaum,
ein `--`-Kommentar ist danach spurlos weg. Mit der Herkunft lautet die Frage
beim Planen nicht mehr „unterscheidet sich der Dateitext von der Katalogform?"
— das tut er immer —, sondern „hat der **Autor** den Text seit dem letzten
Anwenden geaendert?". Das ist eine Aussage ueber zwei Autorentexte und braucht
den Server nicht.
...
**Darstellungs**-gebunden: die Herkunft beschreibt ein Schema, keinen Uebergang.

Erzeugt wird das Dokument von `schema migrate --execute --provenance-output`,
und **nur nach einem sauberen Post-Compare** — vorher steht nicht fest, dass
das Paar zusammengehoert. Gelesen wird es beim naechsten Lauf ueber
`--migration-overlay`. Ohne Herkunft wird konservativ geplant: die Aenderung
wird ausgefuehrt, auch wenn sie vielleicht unnoetig ist.
...
| `--fetch-size` | Nein | Integer | Dialekt-Default (`1000`) | JDBC-Cursor-Prefetch für den Quell-Read ([`LN-005`](lastenheft-d-migrate.md#ln-005)). Für sehr breite Zeilen kleiner wählen, um die Peak-Speicherlast zu senken. Präzedenz: CLI-explizit > `pipeline.fetch_size` (Config) > Dialekt-Default; `≤ 0` → Exit `2`. SQLite: nur Hint (kein serverseitiger Cursor). |
...
| `--chunk-size` | Nein | Integer | `10000` | Datensätze pro Chunk/Transaktion. Präzedenz: CLI-explizit > `pipeline.chunk_size` (Config) > Default; `≤ 0` → Exit `2`. (`--fetch-size` gilt nicht für den Import — er liest aus Format-Dateien, nicht per JDBC-Cursor.) |
...
| `--chunk-size` | Nein | Integer | `10000` | Rows pro Streaming-Chunk. Präzedenz: CLI-explizit > `pipeline.chunk_size` (Config) > Default; `≤ 0` → Exit `2`. |
| `--fetch-size` | Nein | Integer | Dialekt-Default (`1000`) | JDBC-Cursor-Prefetch für den **Quell**-Read ([`LN-005`](lastenheft-d-migrate.md#ln-005)); der `--verify`-Read-Back (Quelle+Ziel) nutzt denselben Wert. Präzedenz: CLI-explizit > `pipeline.fetch_size` (Config) > Dialekt-Default; `≤ 0` → Exit `2`. SQLite: nur Hint. |
