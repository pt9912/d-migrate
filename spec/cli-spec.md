...
**`legacy_serial_syntax` einer Identity-Spalte**: nur PostgreSQL unterscheidet
eine Spalte in der Schreibweise `SERIAL` von einer IDENTITY-Spalte, und nur
sein Reverse setzt das Feld je Spalte. Die uebrigen Reverses setzen es
pauschal — MySQL (`AUTO_INCREMENT`) und SQLite (`AUTOINCREMENT`) immer, SQL
Server und Oracle nie. `schema compare` vergleicht das Feld deshalb nicht,
sobald eine der beiden Seiten aus einem dieser vier Dialekte zurueckgelesen
wurde. Zwei PostgreSQL-Reverses und zwei handgeschriebene Schemata vergleichen
es weiter.

**Grenze — der Identity-Modus gegen SQL Server:** SQL Server kennt kein
`GENERATED BY DEFAULT`. `schema generate` rendert eine solche Spalte als
`IDENTITY(1,1)` und meldet das mit `W140`; der Reverse liest sie als
`mode: always`. Eine `by_default`-Spalte bleibt gegen einen SQL-Server-Reverse
deshalb ein Unterschied im Modus — ein Faehigkeitsunterschied, den
`schema compare` nicht wegdefiniert.
