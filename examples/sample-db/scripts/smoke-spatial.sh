#!/usr/bin/env bash
# Sample-DB-Harness — VA1-Live-Smoke (Spatial, vorgezogenes 5a/5b)
# Plan: docs/planning/in-progress/spatial-harness-slice.md
# ADR:  docs/adr/0014-sample-db-harness-fetch-and-compose.md
#
# Verifiziert die Spatial-VA1-Kette (Geometrie-WERT-Transfer) LIVE gegen echtes
...
#      werden.
#   2. MySQL→MySQL (native): dito mit MySQL-eingebauter Geometrie.
#
...
# docker compose, lokales d-migrate:dev mit VA1 (`make docker-build IMAGE_TAG=dev`).
...
for db in geo_pg_src geo_pg_target; do
    ...
    psql_pg "$db" -c "DROP TABLE IF EXISTS geo; CREATE TABLE geo (id int PRIMARY KEY, name text, g geometry);" > /dev/null
...
log "SUCCESS — VA1 live-verified: geometry value round-trips PG→PG and MySQL→MySQL; native PG point unaffected (R1)."
