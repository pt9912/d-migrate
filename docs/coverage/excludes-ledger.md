...
| `:adapters:driven:driver-sqlite` | `classes` | `dev.dmigrate.driver.sqlite.SqliteAtomicSequencePreserveExecutor*` | `refactor-plan: docs/planning/open/adapter-coverage-uplift.md` | Atomic-preserve executor: `BEGIN IMMEDIATE` + `PRAGMA busy_timeout` + probe/restore covered by `:test:integration-sqlite` (root-Kover-aggregated per E.3); per-module-Gate sees no JDBC test data. |
