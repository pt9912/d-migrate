...
| `:adapters:driving:cli` | `classes` | `dev.dmigrate.cli.Native*` | `permanent: cli-command-shell-pattern` | GraalVM-Native-Image Core-Entrypoint (NativeMain.kt); thin Clikt shells, Kern-Logik in unit-getestetem core/formats (SchemaFileResolver/SchemaValidator). Nur per `nativeCompile` (GraalVM) ausführbar. |
