...
| `:adapters:driven:driver-oracle` | `classes` | `dev.dmigrate.driver.oracle.OracleDriver` | `refactor-plan: docs/planning/next/adapter-coverage-uplift.md` | Driver composition shell; covered through integration paths. |
...
| `:adapters:driven:persistence-jdbc` | `classes` | `dev.dmigrate.server.persistence.jdbc.artifact.JdbcArtifactStore*` | `refactor-plan: docs/planning/next/adapter-coverage-uplift.md` | Postgres-only JDBC adapter (JSONB, INSERT...ON CONFLICT...RETURNING); covered by JdbcArtifactStoreContractTest under `-PintegrationTests`. |
...
| `:adapters:driven:persistence-jdbc` | `classes` | `dev.dmigrate.server.persistence.jdbc.job.JdbcJobStartTransaction*` | `refactor-plan: docs/planning/next/adapter-coverage-uplift.md` | Enthaelt selbst KEIN SQL — reine Cross-Store-Orchestrierung, gekoppelt an die `*OnConnection`-Methoden konkreter Stores. Integrationsgebunden nicht wegen Postgres-Syntax, sondern weil ihr Vertrag die Atomaritaet zweier Schreibvorgaenge in EINER TX ist; ein Mock-Test koennte nur Aufrufreihenfolge behaupten, nicht Rollback beweisen. Gemessen 2026-08-09: 15 offene Zeilen. |
...
| `:adapters:driven:persistence-jdbc` | `classes` | `dev.dmigrate.server.persistence.jdbc.quota.JdbcOwnerAwareQuotaService*` | `refactor-plan: docs/planning/next/adapter-coverage-uplift.md` | Enthaelt selbst KEIN SQL — reine Orchestrierung ueber JdbcQuotaStore/JdbcQuotaReservationOwnerStore. Integrationsgebunden aus demselben Grund wie JdbcJobStartTransaction (Atomaritaet von Statuswechsel + Counter-Update in EINER TX). Gemessen 2026-08-09: 36 offene Zeilen. |
| `:adapters:driven:persistence-jdbc` | `classes` | `dev.dmigrate.server.persistence.jdbc.quota.JdbcQuotaReservationOwnerStore*` | `refactor-plan: docs/planning/next/adapter-coverage-uplift.md` | Enthaelt selbst KEIN SQL — reine Orchestrierung ueber JdbcQuotaStore/JdbcQuotaReservationOwnerStore. Integrationsgebunden aus demselben Grund wie JdbcJobStartTransaction (Atomaritaet von Statuswechsel + Counter-Update in EINER TX). Gemessen 2026-08-09: 36 offene Zeilen. |
| `:adapters:driven:persistence-jdbc` | `classes` | `dev.dmigrate.server.persistence.jdbc.quota.JdbcQuotaStore*` | `refactor-plan: docs/planning/next/adapter-coverage-uplift.md` | Enthaelt selbst KEIN SQL — reine Orchestrierung ueber JdbcQuotaStore/JdbcQuotaReservationOwnerStore. Integrationsgebunden aus demselben Grund wie JdbcJobStartTransaction (Atomaritaet von Statuswechsel + Counter-Update in EINER TX). Gemessen 2026-08-09: 36 offene Zeilen. |
...
| `:adapters:driven:persistence-jdbc` | `classes` | `dev.dmigrate.server.persistence.jdbc.schema.JdbcSchemaStore*` | `refactor-plan: docs/planning/next/adapter-coverage-uplift.md` | Postgres-only JDBC adapter (JSONB, SELECT FOR UPDATE, INSERT...ON CONFLICT...RETURNING); covered by JdbcSchemaStoreContractTest under `-PintegrationTests`. |
| `:adapters:driving:cli` | `classes` | `dev.dmigrate.cli.commands.ConfigCommand*` | `permanent: cli-command-shell-pattern` | Thin Clikt command shell (`config` parent); subcommand wiring + `rootConfigPath` Clikt-context glue. |
