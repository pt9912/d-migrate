...
| `--resume`            | Resume eines frueheren Imports aus einer Checkpoint-Referenz (seit 0.9.0 Phase D.1 produktiv: Manifest-Lifecycle + semantischer Preflight). Wert ist eine `checkpoint-id` oder ein Pfad innerhalb des effektiven Checkpoint-Verzeichnisses. Der Preflight prueft `operationType == IMPORT`, einen Fingerprint ueber alle resume-relevanten Optionen (Format, Encoding, CSV-Header/NULL, `--on-error`/`--on-conflict`/`--trigger-mode`/`--truncate`/`--disable-fk-checks`/`--reseed-sequences`/`chunk-size`, Tabellenliste, Input-Topologie, Input-Pfad, Ziel) sowie die Tabellenlisten-Gleichheit. Nicht unterstuetzt mit stdin (`--source -`) → Exit 2. Ohne konfiguriertes Checkpoint-Verzeichnis → Exit 7. Kompatibilitaetsmismatch → Exit 3. Die ab-Commit-Fortsetzung (tabellengranular + chunk-granular) folgt in Phase D.2/D.3, die stabile `table -> inputFile`-Bindung fuer Directory-Importe in Phase D.4. |
| `--checkpoint-dir`    | Verzeichnis fuer Checkpoints; Vorrang vor `pipeline.checkpoint.directory` aus der Config. |
