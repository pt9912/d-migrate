...
| `3` | Preflight-Fehler — seit 0.9.0 Phase C.1 semantisch inkompatible Resume-Referenz (operationType-Mismatch, Fingerprint-Mismatch aus Format/Encoding/CSV-Optionen/Filter/`--since-*`/Tabellen-Reihenfolge/Output-Modus/Output-Pfad, oder Tabellenliste divergiert). Phase C.2 erweitert den Fingerprint um die PK-Signatur pro Tabelle und mappt den Fall „Manifest hat `resumePosition`, aber aktueller Request hat kein `--since-column`" ebenfalls auf Exit 3. Mapping ist symmetrisch zum Import-Preflight (§4.5); `docs/ImpPlan-0.9.0-C1.md` §4.2 / `docs/ImpPlan-0.9.0-C2.md` §4.1 |
...
| `--resume` | Nein | String | — | Resume eines frueheren Imports aus einer Checkpoint-Referenz (CLI-Vertrag 0.9.0 Phase A; Manifest-Lifecycle + semantischer Preflight seit 0.9.0 Phase D.1, `docs/ImpPlan-0.9.0-D.md`). Wert ist eine `checkpoint-id` **oder** ein Pfad; Pfade MUESSEN innerhalb des effektiven `--checkpoint-dir` / `pipeline.checkpoint.directory` liegen (Pfade ausserhalb → Exit 7). **Nur file-/directory-basiert**: kombiniert mit stdin-Quelle (`--source -`) endet der Aufruf mit Exit 2; ohne konfiguriertes Checkpoint-Verzeichnis → Exit 7. Der Preflight prueft `operationType == IMPORT`, den Options-Fingerprint (Format, Encoding, CSV-Header/NULL, `--on-error`/`--on-conflict`/`--trigger-mode`/`--truncate`/`--disable-fk-checks`/`--reseed-sequences`/`chunk-size`, Tabellenliste in Reihenfolge, Input-Topologie, Input-Pfad, Ziel-Dialekt und Ziel-URL) sowie die Tabellenlisten-Gleichheit. Inkompatible Referenzen → Exit 3. Ab-Commit-Fortsetzung (tabellengranular und chunk-granular) folgt in Phase D.2/D.3; die stabile `table -> inputFile`-Bindung fuer Directory-Importe in Phase D.4. |
| `--checkpoint-dir` | Nein | Pfad | (Config `pipeline.checkpoint.directory`) | Verzeichnis fuer Checkpoints. Der CLI-Wert hat Vorrang vor `pipeline.checkpoint.directory` in `.d-migrate.yaml`. |
...
- `3`: Preflight-Fehler — Header-/Schema-Mismatch, strikter Trigger, und seit 0.9.0 Phase D.1 semantisch inkompatible Resume-Referenz (`operationType`-Mismatch, Fingerprint-Mismatch, Tabellenlisten-Divergenz)
- `4`: Verbindungsfehler
- `5`: Import-Fehler während Verarbeitung oder Commit
- `7`: Konfigurations-, Parse- oder Datei-Fehler (inkl. unlesbare Checkpoint-Datei, ungueltiges Manifest, fehlende Datei, Pfad ausserhalb des Checkpoint-Verzeichnisses oder inkompatible `schemaVersion`, seit 0.9.0 Phase D.1)
