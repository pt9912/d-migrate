...
kommen anschließend per `AddGeometryColumn()` dazu. Eine Spalte mit
`required: true` bekommt dabei ein sechstes Argument (`1`) und entsteht als
`NOT NULL`:
...
- **Auf SpatiaLite geht `required: true` an einer Geometriespalte** — sie
  entsteht als `NOT NULL`, und eine Zeile ohne Geometrie wird abgewiesen. Was
  dort **weiterhin** die ganze Tabelle mit `[E052]` blockiert, ist alles, was
  `AddGeometryColumn()` nicht ausdrücken kann:

  | An der Geometriespalte | Grund |
  |---|---|
  | `unique: true` | die Funktion legt keinen Unique-Index an |
  | ein `default` | sie nimmt keinen Default entgegen |
  | ein Fremdschlüssel (`references`) | sie schreibt keine `REFERENCES`-Klausel |
  | die Spalte im `primary_key` | die Spalte entsteht erst **nach** dem `CREATE TABLE` |
  | ein tabellenweiter Constraint auf ihr | dieselbe Ursache |

  Bei `schema migrate` gilt dieselbe Liste — dort blockiert `[E052]` den
  **ganzen** Plan, nicht nur die Tabelle. Zwei Fälle kommen hinzu: eine
  `required`-Geometriespalte an eine **bestehende** Tabelle anzuhängen bleibt
  blockiert (SpatiaLite füllt die vorhandenen Zeilen mit einem Wert, der keine
  gültige Geometrie ist), und jede Änderung, die SQLite über einen
  Tabellen-Neubau erledigt (Nullbarkeit, Typ, Default, Schlüssel,
  Constraints), blockiert an einer Tabelle mit Geometriespalte — der Neubau
  nähme ihr die SpatiaLite-Registrierung. Legen Sie solche Tabellen in diesem
  Fall neu an und übertragen Sie die Daten.
...
- **SQL Server partitioniert ausschließlich nach `range` über eine Spalte.**
  `list` blockiert dort mit **E055** — der Grund ist keine Einschränkung des
  Werkzeugs: SQL Server kennt nur RANGE.
- **`hash` lässt sich für SQL Server nachbauen**, mit
  `--mssql-hash-partitions computed_column`. Ohne die Option bleibt es beim
  Abbruch mit **E055**. Der Nachbau legt eine berechnete Spalte
  `dmg_hash_bucket` an und partitioniert nach ihr. Drei Dinge sollten Sie
  vorher wissen:
  - **Die Eimerspalte tritt in jeden eindeutigen Schlüssel der Tabelle** — SQL
    Server verlangt die Partitionsspalte dort. Das ändert die Bedeutung des
    Schlüssels nicht, solange die Hash-Spalte selbst darin vorkommt. Tut sie es
    nicht, bricht der Lauf mit **E067** ab, statt Ihnen einen schwächeren
    Schlüssel unterzuschieben.
...
database:
  connections:
    ...
    ms:     "mssql://user:${MS_PW}@host:1433/db"         # Alias: sqlserver; Migrate/Profiling folgen
```
...
- **Die Tabelle ist quotiert klein- oder gemischtgeschrieben** (so legt
  ...
  Zeile deshalb **nicht** von Hand ein. Zwei Wege führen zum Ziel: die SRID in
...
muss. Hat die Tabelle keinen — etwa weil ihr Primärschlüssel zusammengesetzt
ist — bricht der Lauf mit **E070** ab, statt einen Schlüssel zu raten. Und je
Tabelle ist genau **ein** Volltext-Index zulässig; er darf mehrere Spalten
umfassen, zwei getrennte Indizes melden **E071**.
