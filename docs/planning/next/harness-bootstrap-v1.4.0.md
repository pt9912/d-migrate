# Plan: Harness-Bootstrap auf Regelwerk v1.4.0
...
> User-bestätigt) führte die Pflicht-Artefakte + Durchsetzungsschicht ein.
> Seit **2026-06-28 ist v1.4.0 die autoritative Version** — inhaltlich
> **identisch** zu v1.3.0 bis auf den Versionsstempel in den Quell-URLs
> (`blob/v1.3.0/`→`blob/v1.4.0/`, diff-verifiziert 2026-06-28); die
> Bestandsaufnahme gilt unverändert. Methodik-Quelle ist extern
> (`pt9912/ai-harness-course`, Release-ZIP `v1.4.0`), nicht im Repo.
...
v1.4.0 des externen Regelwerks fordert **Pflicht-Artefakte** und eine
**Durchsetzungsschicht** (seit v1.3.0 unverändert), die dieses Repo noch nicht
trägt. Wichtig für die Einordnung: die **Verzeichnis-Layout-Unterschiede** (unser
`docs/adr/` + `docs/planning/` statt des v1.4.0-Default `docs/plan/adr/` + `docs/plan/planning/`) <!-- d-check:ignore (v1.4.0-Default-Layout, bewusst NICHT adoptiert; ADR 0004/0011) -->
...
werden, sonst gelten sie in v1.4.0 als „stille Setzung" (dieselbe Harness-Lüge-
Klasse wie ein undeklariertes Gate).

## Bestandsaufnahme — konform vs. Lücke

| Bereich | v1.4.0-Erwartung | Repo-Ist | Bewertung |
| ------- | ---------------- | -------- | --------- |
...
- **AP1 — `harness/conventions.md`** (höchster Wert, geringstes Risiko). <!-- d-check:ignore (geplantes Pflicht-Artefakt, existiert noch nicht; ADR 0011) -->
  Legt die Baseline (Regelwerk v1.4.0) fest und **deklariert unsere Adaptionen**
...
- **AP3 — Carveout-Modell-Entscheidung.** Per ADR entscheiden: bei unserem
  ...
  auf das v1.4.0-Pro-Datei-Modell migrieren. Bei Verbleib: `Letzte Prüfung`-
  Frische + Audit-Disziplin nachrüsten (das ist der substanzielle v1.4.0-Mehrwert,
...
- Regelwerk v1.4.0 gesichtet (2026-06-28); inhaltsgleich zu v1.3.0 (gesichtet 2026-06-18).
- Sample-DB-Harness (Phase 1) hat Vorrang — dieser Bootstrap startet danach.

## Nicht-Ziel

- **Kein** Big-Bang-Umzug `docs/adr` → `docs/plan/adr` etc.; das Layout bleibt <!-- d-check:ignore (v1.4.0-Default-Layout, bewusst NICHT adoptiert; ADR 0004/0011) -->
