...
| [`harness-bootstrap-v1.3.0.md`](harness-bootstrap-v1.3.0.md) | Bootstrap-Plan (Slice-Bündel) | Adoption der Regelwerk-v1.3.0-Pflichtartefakte: `harness/conventions.md` (deklariert die Layout-Adaption gemäß ADR 0004 + Carveout-/Gate-Adaptionen als `MR-NNN`), `harness/README.md`, Carveout-Modell-Entscheidung, `AGENTS.md`, optional Durchsetzungs-Hooks. Bewusst hinter dem Sample-DB-Harness priorisiert. |
...
| [`ports-jdbc-entkopplung.md`](ports-jdbc-entkopplung.md) | Draft mit Scope | `java.sql` aus `ports-common`/`ports-execute` entfernen: neutrales `DatabaseConnection` (ADR 0022) statt durchgereichter JDBC-`Connection`; JDBC nur noch in Adaptern. Phasen P1-P4 (Typ+JDBC-Impl → Pool → Executor → Detekt-Fitness-Function) + Akzeptanzkriterien; Doku-Folgearbeit (architecture.md + hexagonal-port.md-Archivierung) nachgelagert. |
| [`fulltext-structural-cross-dialect.md`](fulltext-structural-cross-dialect.md) | Draft mit Scope | Strukturelle Cross-Dialect-Uebersetzung des `fulltext`-Typs statt `text`-Degradierung: MySQL `FULLTEXT`-Index, SQLite FTS5-Virtual-Table + Sync-Trigger (SpatiaLite-/ADR 0016-Muster). Phasen P0-P5 + Akzeptanzkriterien; Kernfrage Quelltext-Herleitung (nur im `tsvector_update_trigger`-Body), braucht eigene Slice-ADR. Aus Carve-Out (`carveout.md` Abschnitt 8). |
