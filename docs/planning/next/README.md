...
| [`reverse-preferences.md`](reverse-preferences.md) | Slice-Plan (Scope-Schnitt) | Muster „deklarierte Reverse-Praeferenzen" fuer inhaerente Mehrdeutigkeiten (Erstfall SQLite-AUTOINCREMENT-Breite `identifier`↔`biginteger+identity`): Praeferenz-Feld auf `SchemaReadOptions` + CLI-Flag + `.d-migrate.yaml`-`reverse:`-Block; verankert per neuer ADR + `reverse-preference-mechanism.md` (neu unter spec/). Loest die 64bit-Transfer-Frage OHNE Fingerprint-Fold/v8-Bump. Ersetzt Option 2 aus `open/sqlite-reverse-identifier-64bit-narrowing.md`. Plan-Review erledigt 2026-07-05 (R1-R5 entschieden, F1-F6 eingearbeitet); bau-reif. |