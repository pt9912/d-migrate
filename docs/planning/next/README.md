...
| [`partition-mapping-overlay.md`](partition-mapping-overlay.md) | Draft mit Scope | Dritte Overlay-Art fuer Partitions-Identitaet, die das Werkzeug nicht ableiten kann: Kindnamen (SQL Server nummeriert) und LIST->RANGE (verifizierbar). Traegt die Vorentscheidung E1 — das Overlay-Dokument ist heute an ein Fingerabdruck**paar** gebunden und nur an `schema migrate` verdrahtet, beide Faelle liegen ausserhalb davon. P0-P5 mit Abnahmekriterien; HASH ist Nicht-Scope. |
