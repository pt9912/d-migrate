...
| [`a-check-architecture-gate.md`](a-check-architecture-gate.md) | Draft mit Scope | Hexagon-Schicht-Gate via a-check (extern, v0.12.0, digest-gepinnt) — verallgemeinert `ports-jdbc-free-gate` (ADR 0022) hexagon-weit. Werkzeug + strenge `.a-check.yml` + standalone `make a-check` stehen; Slice behebt die 12 Befunde (5 java.sql + 7 lateral im formats/streaming/parquet-Dreieck) und schaltet a-check dann ins `gates`-Aggregat. |
