...
| [`postcompare-type-canonicalization-slice.md`](postcompare-type-canonicalization-slice.md) | Slice-Plan (Scope-Schnitt 2026-07-03, Review ausstehend) | Dialektbewusste Fingerprint-Kanonisierung im Post-Compare: Typ-Abflachungs-False-Positives (SQLite Exit 5) + Single-Column-UNIQUE-Fold über Round-Trip-Projektion je Ziel-Dialekt, `schema-fingerprint-v6`→`v7`, plus SQLite-Reverse-Fix (inline-UNIQUE wird heute komplett verworfen). Hervorgegangen aus `open/sqlite-postcompare-type-flattening-drift.md`; AP0-AP6 + Abnahme; aktiv beim ersten Implementierungs-Commit. |