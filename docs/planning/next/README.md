...
| [`reader-treue.md`](reader-treue.md) | Umbrella (Schnitt 2026-09-17) | Stille Verluste und Fremdes im Reverse und am Generator, aus einer Konsumentenmessung gegen 1.7.1 und dem Compare-Bau; zwei Muster: Fidelity (ein Verlust wird nicht gesagt) und Modell-Reinheit (der Reader nimmt zu viel auf). Trägt, was die vier Pläne teilen: Nenner, Belegart, Verbleib jedes Postens, Regeln der Matrix-Abnahme (Neu-Pins, betroffene Zellen), Nulllinie der Integrationsmodule, Doku-Pflichten, Code-Tabelle (`R370`, `R371`, `R402`–`R405`, `R221`, `W162`–`W164`). Eigner-Reihenfolge 1 → 2 → 3 → 4. Neue Eigner-Fragen: E1 `"…"`-Bezeichner gegen MySQL (Empfehlung: MySQL-Generator quotiert um), E2 `integer`-Identity-PK beheben, E3 SQLite-Migrate-Identity beheben, Bestätigung der Fingerabdruck-Anhebung für Array-Elemente. Ersetzt den ungeschnittenen Plan (jetzt in `docs/archive/`). |
| [`reader-treue-1-matrix-abnahme.md`](reader-treue-1-matrix-abnahme.md) | Entwurf mit Scope, aktivierungsbereit | Plan 1: P6 MySQL-Server-Text (Introducer, Backslash-Escape, Backtick-Quoting) aus CHECK und Berechnungsausdruck; P0 macht die 5x5-Compare-Matrix zur Abnahme (PostGIS-Dienst im eigenen Schema, angemerkte Seeds, Silent-Loss-Check auf Quell- und Zielseite, `REPORT_CODES_*`/`GEN_CODES_*`); S4 SQLite-Migrate behält FK-Aktionen; P12 SQL-Server-Berechnungsausdruck ohne T-SQL-Quoting; P11 SQLite-Constraint-Namen aus der Quelle, schemaweit eindeutig. Gate: keins; E1 sperrt nur die Neu-Pins von P6 und P12. |
| [`reader-treue-2-meldungen.md`](reader-treue-2-meldungen.md) | Entwurf mit Scope | Plan 2: P5 Array-Verlust auf MySQL **und** SQLite (`W162`), P10 `ALWAYS` ohne Entsprechung (`W163`), P8 PostgreSQL `json` (`R402`), P9 ungebundene Zahl in PostgreSQL, SQLite und Oracle plus stille Fallbacks (`R404`, `R221`, `R371`, `R301`), P1 Oracle-SRID (`R370`, `R365` wird `WARNING`, `W120`-Hinweis je Fall; ADR 0058), P3 `search_path` (`R405`); dazu S1 `integer`-Identity-PK verliert den Modus, S2 SQLite-Migrate rendert keine Identity, S3 PostgreSQL-Generator rendert Array-Elemente als `TEXT[]` (je erst messen). Gate: Plan 1. |
| [`reader-treue-3-spatial.md`](reader-treue-3-spatial.md) | Entwurf mit Scope | Plan 3: P4 PostgreSQL `geography` nur auf dem Rückweg als Geometrie mit SRID (`R403`), samt Datenpfad und `geography`-Seed; vorwärts bleibt `geometry` (F1); P7 SpatiaLite `NOT NULL` nativ in Generate, Migrate (inkl. Rebuild) und Reverse; P2a/P2b Oracle-Spatial-Sequenzen und PostGIS-Routinen und -Sichten stumm filtern, nach Katalog statt Namen. Gate: Plan 1; P4 nach P3. |
| [`reader-treue-4-mssql-berechneter-typ.md`](reader-treue-4-mssql-berechneter-typ.md) | Entwurf mit Scope | Plan 4: P13 der deklarierte Typ einer berechneten SQL-Server-Spalte — erst messen, dann nach der bestätigten Regel: `CAST`-Hülle mit exakter Rückführung im Reader oder `W164`. Gate: P12 aus Plan 1. |
