...
| [`mcp-real-e2e-scope-matrix.md`](mcp-real-e2e-scope-matrix.md) | Draft mit Scope | Kein Test beweist heute, dass MCP-Scope-Denial funktioniert (alle Testprinzipale sind hart admin/voll-scoped). Teil A: generierte Scope-Matrix in `test/e2e-cli` (Real-Subprozess, positiv+negativ pro `DEFAULT_SCOPE_MAPPING`-Eintrag inkl. `connections/list`). Teil B: neuer `examples/mcp-e2e/`-Harness <!-- d-check:ignore (Zielbild: entsteht in Teil B; ADR 0011) --> (docker-compose+Bash, analog `sample-db`/`bi-demo`) gegen das echte Runtime-Image. |