...
| [`oracle-sample-db-leg.md`](oracle-sample-db-leg.md) | Draft mit Scope | Oracle Slice 3b (ADR 0052): Oracle-Leg im Sample-DB-Harness (`examples/sample-db/`), analog dem MSSQL-Leg -- `docker-compose.yml`-Service, `.env`-Variablen, `smoke-cross-pg2ora.sh`(+optional `ora2pg`), gepinnte Notes-/Zeilenzahl-Baseline. Bewusst aus Oracle Slice 3 (Datenpfad) ausgeschnitten. P0-P5 mit Abnahmekriterien. |