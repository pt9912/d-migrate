...
| [`mysql-sequenz-kanonizitaet-hinter-einen-port.md`](mysql-sequenz-kanonizitaet-hinter-einen-port.md) | Next-Plan | Rest von `dialektlogik-in-den-ports`: `MysqlSequenceSupportNaming` bleibt in `hexagon/application`, weil zwei Stellen (`MigrationPreflightPlanner`, `MysqlSequenceCanonicityStage`) dieselben MySQL-Sequenz-Kanonizitaets-Deklarationen ueber denselben Plan-Durchlauf bauen. Ein Port, den `driver-mysql` implementiert, soll die Frage stellen statt beantworten lassen; offene Designfrage zur Benennung (dialektspezifischer Port vs. Erweiterung von `DialectReadCapabilityProvider`). |
...
| [`berechnete-spalten-im-transferpfad.md`](berechnete-spalten-im-transferpfad.md) | Next-Plan (Scope) | Ein `data transfer` in eine Tabelle mit berechneter Spalte kann nie durchlaufen: der Executor baut jeden Chunk aus der **Ziel**-Spaltenliste, die berechnete Spalte steht also unabhaengig von der Quelle drin. Der Port soll sagen, welche Spalten nicht beschreibbar sind, der Transfer sie auslassen und der Preflight es melden (je Tabelle, neuer W-Code). Grenze: `data import` lehnt weiter ab. P1-P4. |
| [`oracle-doppelter-generierungsausdruck.md`](oracle-doppelter-generierungsausdruck.md) | Next-Plan (Scope) | Oracle lehnt zwei berechnete Spalten mit demselben Ausdruck ab (`ORA-54015`) — gemessen auch bei bloss anderen Leerzeichen und getauschten Operanden, und auch beim nachtraeglichen `ADD`. Eine Textgleichheits-Pruefung deckt damit einen von drei Faellen; Weg A (woertlich vorab + Serverfehler benannt) empfohlen, Weg B haengt an der Parser-Entscheidung. Dazu `ORA-01408`: Spaltenindex und Ausdrucks-Index auf derselben Rechnung sind fuer Oracle derselbe Index. P0-P5. |
| [`ddl-comment-rendering.md`](ddl-comment-rendering.md) | Draft mit Scope | Rest aus `ddl-config-block-unimplemented`: `ddl.include_comments` steuert den DDL-Header-Kommentar (`AbstractDdlGenerator.generateHeader()`, eine Stelle, kein SQL-`COMMENT ON`); offene Designfrage dreiwertige Boolean-Praezedenz (CLI-Flag-Paar vs. reiner Config-Schluessel), Tendenz reiner Config-Schluessel. |
| [`postgresql-default-schema-context.md`](postgresql-default-schema-context.md) | Draft mit Scope | Rest aus `ddl-config-block-unimplemented`: `ddl.postgresql.default_schema` braucht einen bislang fehlenden PostgreSQL-Renderkontext; Designfork Weg A (jeden Bezeichner qualifizieren, breit) vs. Weg B (`SET search_path`, schmal aber sitzungsgebunden) — Entscheidung mit Eigner vor dem Bau. |
| [`inline-foreign-keys-mode.md`](inline-foreign-keys-mode.md) | Draft mit Scope | Rest aus `ddl-config-block-unimplemented`: `ddl.inline_foreign_keys` legt den bereits vorhandenen und in PostgreSQL/MSSQL/Oracle bereits implementierten `deferForeignKeys`-Schalter direkt offen statt nur ueber `--split-mode pre-post`; MySQL/SQLite kennen kein Deferred-Constraint-Konzept, `never` muesste dort mit Exit 7 abgelehnt werden. Deutlich kleinerer Schnitt als urspruenglich angenommen. |