...
| [`enum-generate-silent-degradation.md`](enum-generate-silent-degradation.md) | Slice-Plan (Scope-Schnitt) | `enum` degradiert im migrate-Diff-Pfad still zu `TEXT` (values verworfen, kein Diagnostic), waehrend `generate` nativ rendert. Entschieden **Option 2a + W134**: Diff materialisiert die treuen Faelle wie generate (MySQL `ENUM`, PG `refType`-Typreferenz), Inline-Residuen (PG/SQLite) werden mit `W134` laut; **2b** (Inline-Fidelity + Reverse) Folge-Slice. AP1-AP5 + Abnahme; Plan-Review offen. |
