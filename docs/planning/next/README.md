...
| [`action-required-bestimmt-den-ausgang.md`](action-required-bestimmt-den-ausgang.md) | Next-Plan (Vertrag zuerst) | `action_required` bekommt eine Bedeutung im Exit-Code-Vertrag: "das Ergebnis traegt nicht, was die Eingabe verlangt" -> Exit 8, Exit 8 von "Migration" auf "was erzeugt werden sollte" verallgemeinert, `isError` im MCP aus derselben Regel. Default wird der Bruch, der Ausweg ist ein Flag (`--allow-incomplete`). Aus einer Konsumentenmeldung: `schema generate` liess CHECK-Constraint und berechnete Spalte weg und endete mit Exit 0, waehrend `schema migrate` beim selben E053 seit 1.4.0 mit Exit 8 anhaelt. |
