...
| [`generate-implicit-identifier-pk-materialization.md`](generate-implicit-identifier-pk-materialization.md) | Slice-Plan (Scope-Schnitt) | Impliziten `identifier`-PK im Generate materialisieren: MySQL-KEY fuer AUTO_INCREMENT (Error 1075), SQLite-Dedup Inline-vs-Tabellen-PK (Doppel-PK), PG `SERIAL`+`PRIMARY KEY` als Gratis-Effekt — geteilte `EffectivePrimaryKey`-Regel in core (AP1-AP4). Direkter Folge-Slice der Post-Compare-Kanonisierung; Plan-Review offen (R1/R2). |