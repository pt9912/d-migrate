...
| [`ImpPlan-1.2.0-mcp-policy-file-and-connections-list.md`](ImpPlan-1.2.0-mcp-policy-file-and-connections-list.md) | ImpPlan (Draft) | Zwei MCP-Adapter-Luecken: Policy-Regeln fuer `*_start`-Jobs sind in der CLI nicht konfigurierbar (Default hart Deny, kein `--policy-file`); `connections/list` ist nur Scope-reserviert, kein Dispatch-Handler, kein Live-Verbindungstest. Slice A (Policy-Datei) + Slice B (`connections/list` + optionaler Live-Status, Redaction-Design). |