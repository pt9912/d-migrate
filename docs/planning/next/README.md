...
| [`graalvm-native-image-distribution.md`](graalvm-native-image-distribution.md) | Next-Plan (1.0.0-Gate) | GraalVM-Native-Image-Binaries (Linux/macOS/Windows) als zusätzliche Distributionsklasse — das einzige technisch tiefe der drei offenen 1.0.0-Stable-Gates. Phasen mit vorgelagertem Machbarkeits-Spike; Kern-Gabel = volle Adapter-Fläche vs. Core-CLI-Subset (Hadoop/Parquet, AWS-SDK, ICU, Liquibase, sqlite-jdbc-JNI als Risiken); JNA bleibt inert. |
