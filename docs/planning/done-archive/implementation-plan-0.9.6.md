...
- KI-nahe Spezialtools und MCP-Prompts aus `spec/ki-mcp.md` sind
...
`spec/ki-mcp.md` beschreibt bereits:
...
- MCP-Ressourcen fuer:
  ...
  - Diffs
  - Connection-Refs
- strukturierte Fehler-Envelopes mit den in `spec/ki-mcp.md`
  festgelegten Codes
...
- Dokumentationsnachzug in `spec/ki-mcp.md`, `docs/user/guide.md`,
  `spec/cli-spec.md` oder separater MCP-Startdoku, falls der konkrete
...
- erfolgreich angenommene Abbrueche enden im Jobstatus `cancelled`
  gemaess `spec/job-contract.md`

Begruendung:

- `spec/ki-mcp.md` fuehrt `job_cancel` als kontrolliertes Write-Tool
  auf
...
- weil `spec/architecture.md` `ai/` und `testdata/` noch als geplante
...
- 0.9.6 nutzt fuer fachliche Langlaeufer strikt das bestehende
  d-migrate-Jobmodell aus `spec/job-contract.md`
...
`spec/job-contract.md` ist fuer Jobs und Artefakte normativ. 0.9.6
...
- `ApprovalGrant`
  ...
  - `expiresAt`
- `ManagedJob` gemaess `spec/job-contract.md`
  - `jobId`
  - `operation`
  - `status`
  - `createdAt`
...
- `ManagedArtifact` gemaess `spec/job-contract.md`
  - `artifactId`
  - `filename`
  - `contentType`
  - `sizeBytes`
...
- ein Timeout liefert `OPERATION_TIMEOUT`; bei bereits gestarteten Jobs wird
  der Jobstatus gemaess `spec/job-contract.md` aktualisiert und weitere
...
- Antwort ist die aktuelle `ManagedJob`-Projektion gemaess
  `spec/job-contract.md` inklusive terminaler Jobs, `expiresAt`,
...
- nach erfolgreichem Abschluss wird daraus ein Artefakt gemaess
  `spec/job-contract.md`; `mimeType` wird zu `contentType` und
...
- Job- und Artefaktmodelle aus `spec/job-contract.md` verwenden und
...
Detailplan: [`ImpPlan-0.9.6-D.md`](../done-archive/ImpPlan-0.9.6-D.md)
...
- Jobstatus exakt aus `spec/job-contract.md` verwenden:
...
- KI-nahe Spezialtools und MCP-Prompts aus `spec/ki-mcp.md`
