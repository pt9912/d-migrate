...
> **Referenz**: `docs/planning/implementation-plan-0.9.1.md` Abschnitt 1 bis 4,
> Abschnitt 6.1, Abschnitt 7, Abschnitt 8 und Abschnitt 9;
> `docs/user/quality.md`; `spec/cli-spec.md`;
...
Phase A zieht die Sicherheits-Findings aus `docs/user/quality.md` vor die
...
Laut `docs/user/quality.md` sind fuer 0.9.1 vor allem diese Punkte relevant:
...
Seit Erstellung von `docs/user/quality.md` ist bereits ein Teil der
...
- `SqliteDdlGenerator.quoteIdentifier()` und weitere DDL-Generatoren
  ...
  verifizieren**)
- `--filter` ist in `spec/cli-spec.md` bereits als Trusted Input
...
- `constraint.expression` ist in `spec/neutral-model-spec.md` und
  `spec/ddl-generation-rules.md` bereits als Trusted Input dokumentiert
  (**bereits umgesetzt / zu verifizieren**)
...
- die Injection-Flaeche aus `docs/user/quality.md` ist fuer die betroffenen
  Adapter geschlossen
...
Phase A sollte wie in `docs/planning/implementation-plan-0.9.1.md` vorgesehen
als erste 0.9.1-Phase umgesetzt werden.

Begruendung:

- sie schliesst reale Sicherheits-Findings aus `docs/user/quality.md`
