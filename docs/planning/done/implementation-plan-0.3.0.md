...
- Inkrementeller Export (`--incremental` → 0.4.0, LF-013 — gemeinsam mit `data import` in 0.4.0; siehe roadmap.md)
- Checkpoint-Erstellung und Resume (LN-012 → 0.9.0)
...
`spec/architecture.md` §2.2 wird nicht verletzt.
...
`spec/architecture.md` §2.2 bleibt unverletzt.
...
`docs/user/guide.md` und in die Fehlermeldung des `ConnectionUrlParser`.
...
spec/cli-spec.md                                                             # §6.2 `data export` vollständig aktualisieren: alle 0.3.0-Flags (--split-files, --csv-delimiter, --csv-bom, --csv-no-header, --null-string, --tables, --filter, --encoding, --chunk-size) ergänzen, --incremental → 0.4.0 verschieben, Exit-Codes 0/2/4/5/7 (siehe Plan §6.10)
spec/architecture.md                                                         # §2.2 streaming-Dependency, §3.1 DataReader-Signatur (Pool statt Connection), neutrales Daten-Modell in core
spec/connection-config-spec.md                                               # Status: Entwurf → Stand 0.3.0 (URL-Parser + minimaler Named-Connection-Loader umgesetzt; Tabelle "Status pro Feature" nach §6.14)
```
...
Driver-Module ergänzt werden** — `docs/user/releasing.md` §3.4 weist darauf hin.
