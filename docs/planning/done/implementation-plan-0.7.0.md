...
> Referenzen: `docs/planning/roadmap.md` Milestone 0.7.0, `spec/lastenheft-d-migrate.md`
> LF-011 / LF-014, `spec/cli-spec.md` Abschnitt `export flyway` /
> `export liquibase` / `export django` / `export knex`, `spec/design.md`
> Abschnitt Integrations-Export / Migrations-Rollback,
> `spec/ddl-generation-rules.md`, `spec/architecture.md`.
...
- 0.7.0 ist kein Ersatz fuer den spaeteren diff-basierten
  `schema migrate`-/`schema rollback`-Pfad aus `spec/design.md` und
  `spec/cli-spec.md`.
...
- Gleichzeitig ist es auch nicht dasselbe wie das in `spec/design.md`
...
- Die erzeugten Tool-Artefakte repraesentieren den aus einem einzelnen Schema
  ...
  lesen.
- Der in `spec/design.md` beschriebene diff-basierte Migrations-/Rollback-Pfad
...
Die heutige CLI-Skizze in `spec/cli-spec.md` reicht fuer die reale
Umsetzung nicht aus.
...
- `--target` ist fuer alle vier Export-Subcommands Pflicht; die bisherige
  Stub-Default-Annahme in `spec/cli-spec.md` wird fuer 0.7.0 verworfen.
...
- `spec/cli-spec.md` fuer finalen Export-Vertrag nachziehen
- `spec/design.md` fuer Integrations-Export und Rollback-Zielbild
  nachziehen
- `spec/architecture.md` um neuen Integrations-Adapter und CLI-Pfad
  ergaenzen
- `docs/user/releasing.md` um mindestens einen Tool-Export-Smoke erweitern
...
- [ ] `spec/cli-spec.md`, `spec/design.md`, `spec/architecture.md` und
      `docs/user/releasing.md` beschreiben denselben 0.7.0-Vertrag.
...
Die aktuelle Spezifikation in `spec/cli-spec.md` reicht fuer die reale
Umsetzung nicht aus.
