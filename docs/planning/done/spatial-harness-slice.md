...
| **Geometrie-Erkennung im Read-Pfad** | ✅ **VA1b** für PostgreSQL/MySQL/MSSQL, **SpatiaLite** nachgezogen ([`spatial-read-path-geometry-detection.md`](spatial-read-path-geometry-detection.md)) | Metadaten-Vorabfrage markiert die Spalten, `JdbcChunkSequence` überschreibt den Typ; bei SQLite entscheidet `geometry_columns`, nicht der deklarierte Typname |
