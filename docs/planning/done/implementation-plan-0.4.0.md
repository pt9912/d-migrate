...
23. Streaming-Layer-Tests gegen SQLite (direkter `StreamingImporter`,
    ...
    **R5 — Phase-F-Gate**: Phase D gilt **erst dann** als
    ...
    in `docs/planning/0.4.0-phase-d-reorder.md` (mit
...
| Datei | Änderung |
|---|---|
| `spec/cli-spec.md` §6.2 `data import` | REWRITE des bestehenden Kurzblocks: vollständig analog zu 0.3.0-`data export`, alle Flags aus §3.7.1, veraltetes `--resume` entfernen, Output-Resolver (Stdin/SingleFile/Directory), Exit-Code-Matrix 0/1/2/3/4/5/7, Beispiele mit Round-Trip und UPSERT-Import |
| `spec/cli-spec.md` §6.2 `data export` | LF-013-Block ergänzen: `--since-column`, `--since` mit Beispielen |
| `spec/connection-config-spec.md` | Neue Sektion „Status pro Feature" anlegen und die bisher nur im 0.3.0-Plan referenzierte Tabelle dorthin migrieren; dabei `default_source` als aktiv für `data export` und `default_target` als aktiv für `data import` markieren |
| `spec/design-import-sequences-triggers.md` | Status `Draft` → `Approved`; offene Entscheidungen aus §10 mit den Antworten aus §6.6/§6.7/§6.8 dieses Plans befüllen |
| `docs/planning/roadmap.md` | Milestone 0.4.0 als „in Arbeit" markieren bei Phase A; bei jedem abgeschlossenen Phase-Schritt aktualisieren |
