...
> **Status:** Abgeschlossen (2026-09-13) — Leser steht, **alle acht**
> Schlüssel des `ddl:`-Beispiels sind verdrahtet; zwei weitere (`sqlite.*`)
> waren Spec-Dopplung und sind entfernt statt gebaut.
...
7. **Die restlichen drei:**
   ...
   - ~~**`inline_foreign_keys`**~~ — **erledigt 2026-09-13.** War kleiner als
     ...
     [`inline-foreign-keys-mode.md`](inline-foreign-keys-mode.md).
   - ~~**`include_comments`**~~ — **erledigt 2026-09-13.** Bezeichnete nicht
     SQL-`COMMENT ON`, sondern den `-- Generated by …`-Header
     (`AbstractDdlGenerator.generateHeader()`, eine Stelle fuer alle fuenf
     Dialekte). `ddl.include_comments: false` unterdrueckt ihn jetzt; bewusst
     **kein** CLI-Flag (Ziel-Beschreibung wie `ddl.mysql.engine`, kein
     Aufruf-Parameter). Details:
     [`ddl-comment-rendering.md`](ddl-comment-rendering.md).
   - ~~**`postgresql.default_schema`**~~ — **erledigt 2026-09-13.** Weg A
     (jeden Bezeichner qualifizieren) gebaut: umfassender als urspruenglich
     gescoped — nicht nur Tabellen/Indizes/FKs, sondern auch Views,
     Funktionen, Prozeduren, Aggregate, Sequenzen und Custom Types, samt
     jeder Referenz darauf (FK-Ziele, ENUM-`refType`, Sequenz-`nextval()`,
     Trigger-Zieltabellen). Ohne den Schluessel unveraendert unqualifiziert —
     die PostgreSQL-Goldens blieben deshalb unberuehrt. Details:
     [`postgresql-default-schema-context.md`](postgresql-default-schema-context.md).

Damit sind alle acht Schluessel des urspruenglichen `ddl:`-Beispiels
verdrahtet; die zwei zusaetzlich gefundenen `sqlite.*`-Schluessel (Arbeitspaket 6)
waren Spec-Dopplung und sind entfernt statt gebaut. Dieser Tracker ist
geschlossen.
...
[`config-show-full-source-merge.md`](../open/config-show-full-source-merge.md) betrifft
