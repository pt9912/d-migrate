...
> Referenzen: `docs/planning/roadmap.md` Milestone 0.9.0, `spec/lastenheft-d-migrate.md`
> LN-012 und LF-006, `spec/design.md` Abschnitte 3.2, 8.2 und 9.2,
> `spec/architecture.md` (`CheckpointStore`, `I18nConfig`),
> `spec/cli-spec.md` globale Flags / Exit-Codes,
> `spec/connection-config-spec.md` (`pipeline.checkpoint.*`),
> `docs/planning/implementation-plan-0.8.0.md` als I18n-Vorstufe.
...
- `spec/design.md` und `spec/architecture.md` enthalten bereits ein
  konzeptionelles Checkpoint-Modell (`MigrationCheckpoint`,
  `CheckpointStore`), das im Produktionscode noch nicht konkretisiert ist.
- `spec/connection-config-spec.md` dokumentiert `pipeline.checkpoint.*`
...
`docs/planning/ImpPlan-0.9.0-A.md` §4.3 / §3.1; Auflösungs-Semantik prazisiert
in Phase C.1, `docs/planning/ImpPlan-0.9.0-C1.md` §5.2):
...
- `spec/connection-config-spec.md` muss von der vorlaufenden Skizze zu einem
...
- falls dieser Beta-Zuschnitt beibehalten wird, muessen `docs/planning/roadmap.md` und
  `spec/cli-spec.md` die Einschraenkung fuer `stdout`-Export und `stdin`-
...
Die vorhandene Konzeptklasse `MigrationCheckpoint` aus `spec/design.md`
...
`docs/planning/ImpPlan-0.9.0-C.md`):

**Phase C.1 — Export-Resume tabellengranular**
(`docs/planning/ImpPlan-0.9.0-C1.md`):
...
(`docs/planning/ImpPlan-0.9.0-C2.md`):
...
- Single-File-Fortsetzung (Single-Table-Single-File) wird in 0.9.0
  ...
  `spec/cli-spec.md`) und hat daher keinen gueltigen Ausgangslauf, der
...
- Aktualisierung der normativen Doku:
  ...
  - `docs/planning/roadmap.md` fuer eine ggf. explizit dateibasierte 0.9.0-
    Resume-Limitation
  - gezielte Verweise in `spec/design.md`, `spec/architecture.md`,
    `docs/user/guide.md`
...
- Pilot-Testmatrix; fuer spaetere reale Datenbasis siehe
  `docs/planning/test-database-candidates.md`
