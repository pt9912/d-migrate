...
> **Referenz**: `docs/planning/implementation-plan-0.7.0.md` Abschnitt 1, Abschnitt 2,
> Abschnitt 3, Abschnitt 4, Abschnitt 5 Phase A, Abschnitt 6, Abschnitt 7,
> Abschnitt 8, Abschnitt 9, Abschnitt 10; `docs/planning/roadmap.md` Milestone 0.7.0;
> `spec/lastenheft-d-migrate.md` LF-011 / LF-014; `spec/cli-spec.md`;
> `spec/design.md`; `spec/architecture.md`; `spec/ddl-generation-rules.md`
...
- dass die allgemeine CLI-Taxonomie in `spec/cli-spec.md` den realen
...
- `docs/planning/roadmap.md` definiert fuer 0.7.0:
  - Flyway-Adapter
  - Liquibase-Adapter
  - Django-Adapter
  - Knex.js-Adapter
...
- `spec/lastenheft-d-migrate.md` fixiert in LF-011 diese Integrationen als
  optionale Output-Pfade und in LF-014 Rollback als Sollkriterium.
- `spec/cli-spec.md` beschreibt `export flyway` / `export liquibase` /
...
- die globale Exit-Code-Tabelle in `spec/cli-spec.md` reserviert `7`
  ...
  verwenden.
- `spec/design.md` beschreibt Rollback weiterhin im allgemeinen
  Migrationszielbild als diff-basierten Up-/Down-Pfad auf Basis von
  `DiffResult`.
- `spec/cli-spec.md` beschreibt `schema migrate` und `schema rollback`
  weiterhin bewusst als spaeteren Milestone.
- `spec/ddl-generation-rules.md` schreibt im DDL-Header bewusst einen
...
- Bereinigung der allgemeinen CLI-Taxonomie in `spec/cli-spec.md`, nicht nur
...
- Abgleich von `spec/cli-spec.md`, `spec/design.md` und
  `spec/architecture.md` gegen den realen Generate-Iststand
...
Die bisherige Stub-Annahme in `spec/cli-spec.md`, dass `--target` optional mit
...
- `spec/cli-spec.md` Abschnitt 2 zieht die allgemeine Semantik von Exit `7`
...
- Der nicht-deterministische DDL-Header-Timestamp aus
  `spec/ddl-generation-rules.md` wird in 0.7.0-Tool-Artefakten nicht
...
- allgemeine CLI-Taxonomie in `spec/cli-spec.md` auf den realen Ist-Root
...
- globale Exit-Code-Tabelle in `spec/cli-spec.md` Abschnitt 2 auf dieselbe
...
- `spec/cli-spec.md` auf den finalen Export-Vertrag ziehen
- `spec/cli-spec.md` §1.1 und §2 ebenso bereinigen wie den Export-Unterabschnitt
- `spec/design.md` fuer baseline-/full-state-Export versus spaeteres
  diff-basiertes Migrationszielbild sauber abgrenzen
- `spec/ddl-generation-rules.md` oder die unmittelbar referenzierte Export-
  Doku auf die inhaltliche Determinismus-Regel fuer Tool-Exporte ziehen
- `spec/architecture.md` nur soweit nachziehen, dass die spaeteren Phasen
...
- [ ] `spec/cli-spec.md` §1.1 behauptet nicht mehr, dass heute bereits nicht
...
- [ ] Die globale Exit-Code-Tabelle in `spec/cli-spec.md` kollidiert dabei
...
- [ ] `spec/cli-spec.md`, `spec/design.md`, `spec/architecture.md` und der
...
rg -n "export flyway|export liquibase|export django|export knex|schema migrate|schema rollback|Commands|Exit-Codes" \
  docs/planning/implementation-plan-0.7.0.md spec/cli-spec.md spec/design.md spec/architecture.md
```
...
rg -n -- "--target|--version|--spatial-profile|--generate-rollback|--report" \
  docs/planning/implementation-plan-0.7.0.md spec/cli-spec.md
```

3. Pruefung der globalen CLI-Taxonomie und Exit-Code-Tabelle gegen den Ist-Code:

```bash
sed -n '1,30p' spec/cli-spec.md
sed -n '64,82p' spec/cli-spec.md
...
```bash
sed -n '24,40p' spec/ddl-generation-rules.md
...
- `spec/cli-spec.md` §1.1 beschreibt keine Root-Taxonomie mehr, die dem
...
- `spec/design.md` unterscheidet Rollback fuer 0.7.0 von spaeterem
