...
| [`security-gates-not-in-ci.md`](security-gates-not-in-ci.md) | Befund mit Erhebung / Trigger Watch | semgrep und a-check stehen in `make gates`, laufen aber in KEINEM Workflow; zeitgesteuert laeuft nichts Sicherheitsrelevantes. `dependency-submission` war fuenf Wochen defekt (behoben `66a27d99`) — ADR 0039 nennt alle drei als 1.0.0-Interimslatte. Entscheidung offen: genuegt Dependabot, oder unabhaengiger Scanner im Nightly (OSV/Trivy/Grype)? |
| [`dependency-cve-exposure-shipped-artifact.md`](dependency-cve-exposure-shipped-artifact.md) | Befund mit Erhebung / Trigger Watch | **90 real verwundbare Abhaengigkeitsversionen im publizierten `1.0.0`-Image** (1 kritisch, 43 hoch), gemessen an `/opt/d-migrate/lib` statt am Graphen — von 110 Alerts betreffen 20 nichts Ausgeliefertes. 33 der 44 kritischen/hohen sind **Netty 4.1.100.Final** aus dem Hadoop-Baum unter `formats-parquet` (38 von 140 MiB, inkl. YARN- und HDFS-Client). **Weg 1 + 2 umgesetzt 2026-08-15: 90 → 4, davon 0 kritisch und 0 hoch** (vorher 1 und 43); Artefakt 240 → 187 Jars, 143 → 112 MB. Eigene Pins gehoben (PostgreSQL 42.7.12, Jackson 2.21.5, Logback 1.5.34), `io.netty`/ZooKeeper/BouncyCastle/YARN/HDFS aus den Hadoop-Deps ausgeschlossen, `commons-beanutils`/`aircompressor` per Constraint gehoben. Funktional ueber die ausgelieferte CLI verifiziert. **Offen**: Weg 3 (Hadoop-Pfad ersetzen) und der transitive Rest — `guava` 27→32 bewusst nicht nebenbei mitgenommen. |
| [`parquet-single-file-import-type-cast.md`](parquet-single-file-import-type-cast.md) | Befund mit Repro / Trigger Watch | `data import --format parquet --source datei.parquet --table t` bricht mit einer durchgereichten `ClassCastException` ab (`IntegerValue` -> `BinaryValue`) und schreibt nichts. **Vorbestehend im veroeffentlichten 1.0.0**, per Kontrolllauf gegen das unveraenderte Image belegt — nicht vom Hadoop-Ausschluss verursacht. Der Verzeichnis-Import mit `manifest.yaml` laeuft fehlerfrei; ohne Manifest werden die Spaltentypen falsch abgeleitet. Entscheidung offen: Typen aus dem Parquet-Schema lesen oder den Aufruf ohne Manifest verstaendlich ablehnen. |
