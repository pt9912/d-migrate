...
| [`import-throughput-parallel.md`](import-throughput-parallel.md) | Vorschlag (Trigger Watch) | Orthogonale Achse zum (erledigten) COPY-Ticket: **paralleler Import** (mehrere gleichzeitige Streams). Offene Korrektheitsfragen vor Scope: **FK-/Ladereihenfolge** über gleichzeitig geladene Tabellen, globaler `triggersDisabled`-Zustand + Pool-Größe, Sättigungsdecke der einen Ziel-Instanz. Aktiviert bei Volumen-Migrationen jenseits des Einzel-Stream-Durchsatzes. |
