...
| [`fulltext-structural-cross-dialect.md`](fulltext-structural-cross-dialect.md) | Vorschlag (Draft) | Strukturelle Cross-Dialect-Übersetzung des `fulltext`-Typs statt `text`-Degradierung: MySQL `FULLTEXT`-Index, SQLite FTS5-Virtual-Table + Sync-Trigger (nach dem SpatiaLite-Muster). Aus Carve-Out (`carveout.md` Abschnitt 8); Kernfrage = Quelltext-Herleitung (steht nur im `tsvector_update_trigger`-Body), braucht eigene ADR. |
