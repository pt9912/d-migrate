...
| [`spatialite-migrate-roundtrip.md`](spatialite-migrate-roundtrip.md) | Vorabklaerung | VA4-Restbefunde fuer den vollen SpatiaLite-`migrate --execute`-Round-Trip (5d): `InitSpatialMetaData()`-Bootstrap fehlt + Diff-`CreateTable` rendert Geometrie-Index als normalen `CREATE INDEX` statt `CreateSpatialIndex`. |
