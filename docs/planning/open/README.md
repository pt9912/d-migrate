...
| [`partition-hierarchy-reconstruction.md`](partition-hierarchy-reconstruction.md) | Vorabklärung (Trigger) | Volle Partitions-Hierarchie-Rekonstruktion (PG zuerst): Kern-Lücke ist **Reverse-Capture** der Kind-Partitionen (`pg_inherits` + `relpartbound`) + ein **partitions-bewusster Comparator** (heute blind); Generate emittiert `PARTITION OF` bereits. Schließt die via `E055` gemeldete Grenze (Pagila `payment`); LN-008-Teillücke. |
...
| [`migrate-postcompare-identifier-pk-drift.md`](migrate-postcompare-identifier-pk-drift.md) | Vorabklaerung (P2) | `migrate --execute` meldet Post-Compare-Drift (Exit 5) bei Schemas OHNE explizites `primary_key` (impliziter `identifier`-PK), weil der Fingerprint die `identifier`→PK-Aequivalenz nicht kanonisiert; **dialektneutral** (PG/MySQL/SQLite), zuerst beim 5d-Live-Apply beobachtet. Explizites `primary_key` = gruen. |
