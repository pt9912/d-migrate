...
| [`sqlite-fulltext-rebuild-recreate.md`](sqlite-fulltext-rebuild-recreate.md) | Vorschlag (Draft, Trigger-Watch) | Punkt 2: den SQLite-Table-**Rebuild**-Pfad (`SqliteRebuildRenderer`) die FTS5-FULLTEXT-Objekte (Virtual-Table + 3 Sync-Trigger) als abhängig droppen+recreaten lassen statt via `createIndexSql` zu degradieren (löst den gelieferten Interim-Block [`../done/sqlite-fulltext-rebuild-block.md`](../done/sqlite-fulltext-rebuild-block.md) ab, inkl. Rückbau des `SQLITE_REBUILD_FULLTEXT_UNSUPPORTED`-Guards). Aus dem P5-Review des Fulltext-Slice (ADR 0025); inkrementelle ALTER-Kante, nicht im Slice-DoD/Akzeptanz. |
| [`sqlite-postcompare-type-flattening-drift.md`](sqlite-postcompare-type-flattening-drift.md) | Vorabklärung | Frisches `migrate --execute` auf SQLite endet Exit 5 (Drift-False-Positive) für jeden Neutraltyp, den `SqliteTypeMapper` abflacht (`smallint`/`biginteger`/`boolean`→INTEGER, `datetime`/`decimal`/… → TEXT/REAL) — Post-Compare vergleicht Neutraltypen wörtlich. Richtung: dialektbewusste Typ-Kanonisierung (M2-Substrat `StructuralTransferTypeCompatibility`, Präzedenz Fingerprint v3). Live belegt 2026-07-02 beim Fulltext-Rebuild-Block. |
