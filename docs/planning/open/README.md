...
| [`sqlite-fulltext-rebuild-recreate.md`](sqlite-fulltext-rebuild-recreate.md) | Vorschlag (Draft) | Den SQLite-Table-**Rebuild**-Pfad (`SqliteRebuildRenderer`) die FTS5-FULLTEXT-Objekte (Virtual-Table + 3 Sync-Trigger) als abhängig droppen+recreaten lassen statt via `createIndexSql` zu degradieren. Aus dem P5-Review des Fulltext-Slice (ADR 0025); inkrementelle ALTER-Kante, nicht im Slice-DoD/Akzeptanz. |
