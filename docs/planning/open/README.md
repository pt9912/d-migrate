...
| [`security-gates-not-in-ci.md`](security-gates-not-in-ci.md) | Befund mit Erhebung / **beide Anliegen erledigt** | semgrep und a-check stehen in `make gates`, laufen aber in KEINEM Workflow; zeitgesteuert laeuft nichts Sicherheitsrelevantes. `dependency-submission` war fuenf Wochen defekt (behoben `66a27d99`) — ADR 0039 nennt alle drei als 1.0.0-Interimslatte. **Erledigt**: semgrep + a-check als Job `security-gates` in build.yml (2026-08-15); zeitgesteuerter Teil als **Trivy**-Nightly `image-scan.yml` gegen die publizierten `:latest`/`:native` (2026-08-16, Eigner-Entscheidung). Trivy fand sofort etwas, das Dependabot strukturell NICHT sieht: geshadetes Jackson 2.21.3 in `parquet-jackson`. |
