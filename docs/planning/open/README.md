...
| [`spec-adr-downref-hygiene.md`](spec-adr-downref-hygiene.md) | Tracker (Schuld behoben) | Spec→ADR/Plan-Abwärts-Refs (`cli-spec:302`, `ledger` „ADR 0020", `architecture` ADR-001..010-Tabelle, `ddl-generation` Plan-2) **behoben 2026-06-28** (Re-Scan leer); offen nur noch die zurückgestellte `.d-check.yml`-Härtung (`ids`/`matrix`). |
