...
| [`import-throughput-copy-path.md`](import-throughput-copy-path.md) | **Schritt 0 erledigt; COPY offen** | PG-**Import** war ~5,4× langsamer als die COPY-Decke (#2-Vergleich). **Schritt 0** (`reWriteBatchedInserts=true`, Pendant zu MySQL) **erledigt + gemessen** (Import ~66,6k→~115,5k rows/s, ~1,73×, verlustfrei). **Offen:** COPY-Fast-Path (`CopyManager`) nur für wrapping-freie Tabellen mit `ON CONFLICT ABORT` — einziger echter COPY-Blocker ist das Geometrie-SQL-Wrapping (`ST_GeomFromWKB`); größerer Lift (Korrektheits-Sperren + neuer Dialekt-Hook). Nicht LF-blockierend. |
