...
| [`keychain-native-provider-module.md`](keychain-native-provider-module.md) | Vorschlag (Draft) / Trigger Watch | Opt-in-Modul `keychain-native` (Windows Credential Manager/DPAPI + optionale native Parität macOS/Linux) auf demselben `KeychainBackend`-Port. Der `keychain:`-Kern (macOS/Linux, native-frei) ist geliefert; Native ist per ADR 0040 opt-in. Braucht Mechanismus-Entscheidung (JNA vs. Keyring-Lib), GraalVM-Native-Image-Config + eigenen Security-Review. Abgespalten aus dem graduierten Plan. |
