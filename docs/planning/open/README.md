...
| [`spec-adr-downref-hygiene.md`](spec-adr-downref-hygiene.md) | Sammlung/Tracker | Spec→ADR-Abwärts-Verweise (`cli-spec:302` bare Pfad, `ledger` „ADR 0020"-Textnennungen, `architecture` ADR-001..010-Tabelle) gegen „Specs verweisen nie auf ADRs" bereinigen + optionale `.d-check.yml`-Härtung (`ids`/`matrix`). Aus dem Milestone-Hygiene-Slice; Gate-Teil bewusst zurückgestellt. |
