...
| [`sample-db-roundtrip-findings.md`](sample-db-roundtrip-findings.md) | Sammlung/Tracker | Vom Sample-DB-Harness (Pagila/PG Phase 1) aufgedeckte Round-Trip-Defekte: F1 Trigger-Naming (M1-Klasse, Generate-Pfad **behoben**; Migrate-Pfad offen), F2 Programmability-Ordering Views vor Routinen (K2-Klasse), F3 Funktions-Attribut-Verlust (Param-Namen + IMMUTABLE/STRICT), F4 Multi-Event-Trigger nicht modelliert; plus zwei fundamentale Grenzen (tsvector/W123, leere RANGE-Partition/E055). |
