...
| [`import-throughput-copy-path.md`](import-throughput-copy-path.md) | Vorabklärung (Trigger) | PG-**Import** ~5,4× langsamer als die COPY-Decke (#2-Vergleich): Default-Pfad = Einzelzeilen-Prepared-`INSERT` + `executeBatch`, und der PG-Writer setzt pgjdbcs `reWriteBatchedInserts` nicht (MySQL hat das Pendant). **Schritt 0** = `reWriteBatchedInserts=true` (risikolos, kein Transform-Konflikt); **danach** COPY-Fast-Path (`CopyManager`) nur für wrapping-freie Tabellen mit `ON CONFLICT ABORT` — der einzige echte COPY-Blocker ist das Geometrie-SQL-Wrapping (`ST_GeomFromWKB`), JSON/Enum/Array sind COPY-darstellbar. Nicht LF-blockierend. |
