...
| [`enum-generate-silent-degradation.md`](enum-generate-silent-degradation.md) | Draft (Trigger Watch) | `enum` rendert auf allen drei Dialekten zu bloßem `TEXT` — `values` verworfen, keine Werte-Durchsetzung im Ziel, Migrate-Report komplett still (widerspricht dem Loud-Prinzip). Scope-Optionen: laute Note (minimal) vs. native Materialisierung (ENUM/CHECK/Custom-Type) mit Reverse-Parität. AP0-Nachprüfung 2026-07-03. |
| [`generate-implicit-identifier-pk-materialization.md`](generate-implicit-identifier-pk-materialization.md) | Draft (Trigger Watch) | Generate materialisiert den impliziten `identifier`-PK nicht: MySQL rendert `AUTO_INCREMENT` ohne KEY (Runtime-Fehler), SQLite rendert bei `identifier` + explizitem `primary_key` doppeltes PRIMARY KEY (SQLITE_ERROR) — Runtime-Execution-Fehler auf spec-validen Schemata. AP0-Befund des Typ-Kanonisierungs-Slices (2026-07-03). |
