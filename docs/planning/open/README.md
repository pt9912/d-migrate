...
| [`sqlite-postcompare-type-flattening-drift.md`](sqlite-postcompare-type-flattening-drift.md) | **AKTIVIERT (nächster Slice, Start 2026-07-03)** | Frisches `migrate --execute` auf SQLite endet Exit 5 (Drift-False-Positive) für jeden Neutraltyp, den `SqliteTypeMapper` abflacht (`smallint`/`biginteger`/`boolean`/`datetime`/`decimal` live geprobt) + UNIQUE-Constraint-Asymmetrie. Richtung: dialektbewusste Typ-Kanonisierung NUR im Post-Compare (M2-Substrat `StructuralTransferTypeCompatibility`, Präzedenz Fingerprint v3, Version-Bump). Einstiegsplan im Ticket unter „Geplanter Einstieg“; erster Schritt = Scope-Schnitt. |
