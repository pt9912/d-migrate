...
| [`sqlite-fulltext-rebuild-block.md`](sqlite-fulltext-rebuild-block.md) | Vorschlag (Draft, loud-safe Interim) | Punkt 1: einen SQLite-Table-Rebuild einer FULLTEXT-tragenden Tabelle **blocken** (`MANUAL_ACTION_REQUIRED`) statt still mit veralteter FTS5 + grünem `migrate` durchzulaufen. Sicherheitsnetz bis zum vollen Recreate; aus dem P5-Review des Fulltext-Slice (ADR 0025). |
| [`sqlite-fulltext-rebuild-recreate.md`](sqlite-fulltext-rebuild-recreate.md) | Vorschlag (Draft, Trigger-Watch) | Punkt 2: den SQLite-Table-**Rebuild**-Pfad (`SqliteRebuildRenderer`) die FTS5-FULLTEXT-Objekte (Virtual-Table + 3 Sync-Trigger) als abhängig droppen+recreaten lassen statt via `createIndexSql` zu degradieren (löst den Interim-Block ab). Aus dem P5-Review des Fulltext-Slice (ADR 0025); inkrementelle ALTER-Kante, nicht im Slice-DoD/Akzeptanz. |
