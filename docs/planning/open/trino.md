...
- URL-Parsing `trino://user@host:port/catalog/schema`
...
- `schema` ist der zweite Pfadteil (optional, falls sinnvoller Default)
- Query-Parameter werden für Phase 1 nicht als Muss-Pfad behandelt
...
d-migrate schema reverse \
  --source db:trino://analyst@localhost:8080/iceberg/default \
  --output lakehouse.yaml

d-migrate schema compare \
  --source file:lakehouse.yaml \
  --target db:trino://analyst@localhost:8080/postgresql/public
...
- `spec/cli-spec.md` (Source/Target-Dialekt-Doku)
...
- `schema compare` läuft gegen `db:trino://...` und gibt klare Diff-/Limit-Warnungen.
...
- [ ] `schema compare --source file... --target db:trino...` lauffähig.
...
- [ ] Mindestens ein Smoke-Testfall ist im Plan verankert.
