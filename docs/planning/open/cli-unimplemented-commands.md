...
`data seed` ist seit der P1-Umsetzung
([`cli-data-seed.md`](../in-progress/cli-data-seed.md),
[`ImpPlan-1.3.0-cli-data-seed-p1.md`](../done/ImpPlan-1.3.0-cli-data-seed-p1.md))
registriert (`--schema`/`--target`/`--count`/`--seed`/`--locale`); `--rules`
(P2) und `--ai-backend` (P3) sind bewusst noch keine Flags des Befehls
(No-Carveouts).
