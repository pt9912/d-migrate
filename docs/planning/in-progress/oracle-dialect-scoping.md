...
| **3b** ⚠ | Sample-db-Oracle-Leg: Compose-Service, `.env`, Verbindung, `smoke-cross-pg2ora.sh`, `make sample-db-cross-smoke-pg2ora` — **gebaut, aber rot**. Der Lauf kommt bis DDL-Anwendung per `sqlplus` und Tabellenzahl-Paritaet (15 = 15) und bricht im Transfer ab: Oracle setzt `''` mit NULL gleich, Pagila fuehrt in zwei `NOT NULL`-Textspalten fuenf leere Strings. Entscheidung offen, siehe [`oracle-empty-string-is-null-transfer.md`](../open/oracle-empty-string-is-null-transfer.md). Der Lauf hat dabei **fuenf Defekte im ausgelieferten Generate-Pfad** aufgedeckt, alle behoben ([`oracle-sample-db-leg.md`](../next/oracle-sample-db-leg.md)) | Harness-Anschluss steht; Cross-Dialekt-Beleg erst nach der Entscheidung |
...
| **4b** ⛔ | Cross-Dialekt-sample-db-Smoke — **blockiert**. Der Harness-Anschluss aus 3b steht inzwischen; blockierend ist jetzt ausschliesslich, dass der Transfer nach Oracle an leeren Strings in `NOT NULL`-Spalten scheitert ([`oracle-empty-string-is-null-transfer.md`](../open/oracle-empty-string-is-null-transfer.md)). 4b setzt einen durchlaufenden Transfer voraus und kann vor dieser Entscheidung nicht beginnen | Cross-Dialekt-Beleg im Harness |
