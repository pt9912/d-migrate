...
| [`oracle-dialect-scoping.md`](oracle-dialect-scoping.md) | Per-Feature-Umbrella | Oracle als fuenfter Dialekt (Milestone 1.8.0, [ADR 0052](../../adr/0052-oracle-fuenfter-dialekt-scoping.md)). Fuenf Grundsatzentscheidungen getroffen (Oracle 23ai Free/`gvenzl/oracle-free`, keine Carve-Outs, FUTC-JDBC-Lizenz-Compliance, CI in jedem Lauf, PL/SQL-Packages ohne Liefertermin). **Geliefert: Slices 0-3b, 4a/4b und 5 (5a-5e)** — `schema migrate` ist fuer Oracle nutzbar, der Sample-DB-Harness faehrt Pagila in beide Richtungen, und Oracle ist im Cross-Dialekt-Matrix-Sweep. Offen: die Ausbau-Slices 6-11; `DialectCommandGate` fuehrt nur noch `data profile`. |
