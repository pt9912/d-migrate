...
| [`reader-treue.md`](reader-treue.md) | Per-Feature-Umbrella (vier Plaene) | Stille Verluste und Fremdes im Reverse und am Generator. Traegt, was die vier Plaene teilen: gemeinsamer Nenner, Belegart, Verbleib jedes Postens, Regeln der Matrix-Abnahme (Neu-Pins einzeln, betroffene Zellen), Nulllinie der Integrationsmodule, Doku-Pflichten, Code-Tabelle (`R370`, `R371`, `R402`-`R405`, `R221`, `W162`-`W164`), entschiedene Eigner-Fragen (E1 = (b), E2, E3, Fingerabdruck-Anhebung). Reihenfolge 1 -> 2 -> 3 -> 4; Plan 1 ist in Arbeit, die Plaene 2-4 stehen in `../next/`. |
| [`reader-treue-1-matrix-abnahme.md`](reader-treue-1-matrix-abnahme.md) | Per-Slice-ImpPlan (Plan 1 des Umbrellas) | P6 MySQL-Server-Text (Introducer, Backslash-Escape, Backtick-Quoting) aus CHECK und Berechnungsausdruck; P0 macht die 5x5-Compare-Matrix zur Abnahme (PostGIS im eigenen Schema, angemerkte Seeds, Silent-Loss-Check, `REPORT_CODES_*`/`GEN_CODES_*`); S4 SQLite-Migrate behaelt FK-Aktionen; P12 SQL-Server-Berechnungsausdruck ohne T-SQL-Quoting; P11 SQLite-Constraint-Namen aus der Quelle. Vorgeschaltet der E1-Bauteil: der MySQL-Generator schreibt `"…"`-Bezeichner in rohen Ausdruecken in Backticks um. |
