...
| [`ImpPlan-0.9.8-object-storage-s3.md`](ImpPlan-0.9.8-object-storage-s3.md) | ImpPlan | Object-Storage S3-Adapter (`adapters:driven:storage-s3`): S3.0-S3.3 done (Gate GO; ContentStore + SegmentStore + Shared-Support, reviewt/konvergiert), aktiv S3.4 (Wiring + E2E). |
| [`object-storage-s3-eval.md`](object-storage-s3-eval.md) | Entscheidungsvorlage | S3-Client-Lib-Verdict: AWS SDK v2 + `url-connection-client`, gegen SeaweedFS validiert; MinIO verworfen (EOL + Native-Image). |
| [`object-storage-artifact-store.md`](object-storage-artifact-store.md) | Architekturplan | Reconciliation gegen die 0.9.6-Byte-Stores (`ArtifactContentStore`/`UploadSegmentStore`); offener Scope auf den S3-Adapter eingeengt. |
