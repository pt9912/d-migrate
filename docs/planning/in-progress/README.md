...
| [`action-required-bestimmt-den-ausgang.md`](action-required-bestimmt-den-ausgang.md) | Next-Plan (Weg beta) | `action_required` bekommt eine Bedeutung im Exit-Code-Vertrag: die Ausgangsregel haengt an `SkippedObject`, nicht an der Notiz-Stufe. P0 abgeschlossen — sechs Emissionsstellen trugen kein `SkippedObject`, obwohl das Objekt wirklich fehlte (darunter die berechnete Spalte UND die CHECK-Constraint aus dem urspruenglichen Konsumentenbefund); alle sechs geschlossen und live belegt. P1 (Ausgangsregel + `--allow-incomplete`), P2 (MCP), P3 (weitere Kommandos), P4 (Doku) offen. |
