...
| [`oracle-dialect-scoping.md`](oracle-dialect-scoping.md) | Per-Feature-Umbrella | Oracle als fuenfter Dialekt (Milestone 1.8.0, [ADR 0052](../../adr/0052-oracle-fuenfter-dialekt-scoping.md)). Fuenf Grundsatzentscheidungen getroffen (Oracle 23ai Free/`gvenzl/oracle-free`, keine Carve-Outs, FUTC-JDBC-Lizenz-Compliance, CI in jedem Lauf, PL/SQL-Packages ohne Liefertermin). **Geliefert: Slices 0-11** — Oracle fuehrt denselben Befehlsumfang wie die vier anderen Dialekte, der Sample-DB-Harness faehrt Pagila in beide Richtungen, und Oracle ist im Cross-Dialekt-Matrix-Sweep. `DialectCommandGate` entfaellt mit seinem letzten Eintrag. Offen: Slice 12 (Spatial) und PL/SQL-Packages ohne Liefertermin. |
| [`partition-mapping-overlay.md`](partition-mapping-overlay.md) | Per-Feature-Slice | Dritte Overlay-Art fuer Partitions-Identitaet, die das Werkzeug nicht ableiten kann: Kindnamen (SQL Server nummeriert) und LIST->RANGE (verifizierbar). **P0** (Bindungsfrage, [ADR 0050](../../adr/0050-overlay-bindung-uebergang-vs-darstellung.md)) und **P1** (sealed `Transition`/`Representation`, `migration-overlay.v2`) geliefert; P2-P7 offen. HASH ist Nicht-Scope. |
