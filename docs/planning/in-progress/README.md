...
| [`reader-treue.md`](reader-treue.md) | Per-Feature-Umbrella (vier Plaene) | Stille Verluste und Fremdes im Reverse und am Generator. Traegt, was die vier Plaene teilen: gemeinsamer Nenner, Belegart, Verbleib jedes Postens, Regeln der Matrix-Abnahme (Neu-Pins einzeln, betroffene Zellen), Nulllinie der Integrationsmodule, Doku-Pflichten, Code-Tabelle (`R370`, `R371`, `R402`-`R405`, `R221`, `W162`-`W164`), entschiedene Eigner-Fragen (E1 = (b), E2, E3, Fingerabdruck-Anhebung). Reihenfolge 1 -> 2 -> 3 -> 4; Plan 1 ist geliefert und graduiert ([`../done/reader-treue-1-matrix-abnahme.md`](../done/reader-treue-1-matrix-abnahme.md)), Plan 2 ist aktiv (siehe naechste Zeile), die Plaene 3 und 4 stehen in `../next/`. |
| [`reader-treue-2-meldungen.md`](reader-treue-2-meldungen.md) | Slice (Plan 2 des Reader-Umbrellas) | Verluste werden gemeldet: P5 Array-Verlust auf MySQL **und** SQLite (`W162`), P10 `ALWAYS` ohne Entsprechung (`W163`), P8 PostgreSQL `json` (`R402`), P9 ungebundene Zahl in PostgreSQL, SQLite und Oracle plus stille Fallbacks (`R404`, `R221`, `R371`, `R301`), P1 Oracle-SRID (`R370`, `R365` wird `WARNING`, `W120`-Hinweis je Fall; ADR 0058), P3 `search_path` (`R405`); dazu die drei gemessenen stillen Verluste S1 (`integer`-Identity-PK behaelt den Modus), S2 (SQLite-Migrate rendert `generation: identity`) und S3 (PostgreSQL-Generator rendert Array-Elemente, Fingerabdruck `v16` -> `v17`). Abnahme: Compare-Matrix und die Integrationsmodule je Dialekt. |
