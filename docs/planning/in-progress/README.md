...
| [`konsumentenbefunde-170-skipped-schemaref-views.md`](konsumentenbefunde-170-skipped-schemaref-views.md) | Per-Slice-Closure (in Arbeit) | Oracle zaehlt Objektverluste als `skipped_objects`, `schemaRef` braucht kein `format` mehr, der View-Vergleich traegt Spalten und ein Semikolon — **ausgeliefert in 1.7.1**. 2026-09-16 aktiviert, weil **vier DoD-Punkte offen** sind: die Abdeckung fuer E052 und vier MSSQL-Indexstellen fehlt (gemessen: Ruecknahme bleibt gruen), der CLI-Renderer-Test fuer `columns` fehlt, und P3s Spec-Satz ist nicht eingeloest. Nach dem Schliessen Graduation nach `../done/`. |
