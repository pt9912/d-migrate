...
| S9b | Single-File-Tests: CLI-Preflight (Single-File-Codes), Phase-1/2-Tests, Single-File-Resume, DuckDB-/Arrow-Single-File-KV-Toleranz (AP12 §11). | Vier Test-Familien gruen via `make docker-test MODULES=":hexagon:application :adapters:driven:streaming :adapters:driven:formats-parquet :adapters:driving:cli"` (Phase-1/2 + Single-File-Resume in `hexagon:application`, KV-Toleranz in `formats-parquet`, CLI-Preflight-Codes in `cli`; `:adapters:driven:streaming` laeuft nur als Regressionsschutz mit — **kein** Single-File-eigener Resolver, da der `manifest.yaml`-Sniff Bundle-only ist (S9a), und die Single-File-Format-Inferenz `inferFormatFromExtension` in `hexagon:application` sitzt). |
