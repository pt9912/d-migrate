...
| **8d** | Diff/Migrate | `schema migrate` legt Volltext-Indizes an und baut sie zurück. **Vorarbeit erledigt:** der Fingerabdruck trug den Indexnamen, und der ist bei SQL Server synthetisiert — jeder Volltext-Round-Trip wäre im Post-Compare gedriftet. Am echten Server gemessen (zwei verschiedene Hashes), über `capabilityIndexCanonicalizer` behoben |
