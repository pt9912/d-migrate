...
| **6** (fast) | Gefilterte Indizes (WHERE) + clustered/nonclustered-Steuerung + INCLUDE-Spalten, Reverse + Generate + Diff | für SQL Server und PostgreSQL geschlossen; offen sind die Degradierungs-Meldungen im Diff-Pfad und ein Live-Beleg — siehe unten |
