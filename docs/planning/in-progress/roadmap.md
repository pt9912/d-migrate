...
| Driver  | SQLite-Seq Phase B: Generator — `helper_table`-Modus mit `dmg_sequences` und Trigger-basierter `nextval`-Emulation (kein Stored Function — Logik im Trigger-Body), `BEFORE INSERT`-Trigger pro Spalte. ✅ B.0 `DdlDialectContext`-Refactor (`48c7f01c`) + B.1 `SqliteNamedSequenceMode` + CLI-Plumbing (`84ba7ab7`) abgeschlossen 2026-05-27. Offen: B.2 Validator-Regeln, B.3 helper_table-DDL + `_bi`/`_ai`-Trigger-Paar, B.4 `SequenceCapability`-Defaults flippen.       | —      |
...
#### Aktueller Arbeitsstand 0.9.7 (2026-05-27)
