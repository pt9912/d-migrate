...
| Refactor| Atomic-Preserve Service-Mode Sub-Slice C (Connection-Sub-Pool) + D (Quota-Plumbing) + F (`schema_migrate`-Handler-Skeleton): warten auf externen Trigger (MCP-Migrate-Tool / gRPC 1.1.8 / REST 1.2.0). Pure Server-Mode-Infrastruktur ohne CLI-Nutzen. Plan-Doc: [`../open/atomic-preserve-service-mode.md`](../open/atomic-preserve-service-mode.md) §3.3. MCP-Tool-Vorabklärung 2026-06-03 angelegt: [`../open/mcp-schema-migrate-tool.md`](../open/mcp-schema-migrate-tool.md). | — |
