...
| F.5 Vollscheibe | Sub-Slices A–E der CHECK-/EXCLUDE-Vollscheibe abgeschlossen (Foundation + PG-/MySQL-/SQLite-Renderer + Daten-Preflight inkl. Probes + Stage + Pipeline + Report + CLI). PG rendert CHECK/EXCLUDE nativ; MySQL gated CHECK ueber `MysqlCheckEnforcementCapability` + blockt EXCLUDE; SQLite faedelt CHECK durch Rebuild + blockt EXCLUDE; live-Daten-Preflight liefert PASSED/FAILED/PROBE_RUNTIME_ERROR pro Op und der Renderer-Gate `CheckPreflightGate` setzt sie um. Plan-Doc: `docs/planning/in-progress/ImpPlan-0.9.7-F.5-check-exclude-vollscheibe.md`. Offen: Sub-Slice F (Reversibility + Replace-Vertrag), Sub-Slice G (Closing/DoD). | teilerledigt (2026-05-19) |
