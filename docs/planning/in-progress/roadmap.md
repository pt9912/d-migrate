...
### Milestone 0.9.8 — Analytics- und Storage-Anschluss (Parquet Cut A + Object-Storage-Plan + BI-Demo)
...
| Arch    | ⏳ Object-Storage: Bestehende Artefakt-/Checkpoint-Pfade inventarisieren, minimalen `ArtifactStore`-Port entwerfen, File-Impl als Referenz (Plan-Entwurf; `ArtifactStore`-Port existiert aus 0.9.6-Server)                           | —      |
| Arch    | ⏳ Object-Storage: S3-kompatible Implementierung evaluieren, Konfigurations-/Security-Regeln skizzieren, Migration des MCP-/REST-/gRPC-Jobvertrags auf Artifact-Refs planen ([`object-storage-artifact-store.md`](../next/object-storage-artifact-store.md), Status: Entwurf)         | —      |
...
> Test-Familien sind vollständig. Offen ist hier nur noch der
> **Umbrella-Closure** (CHANGELOG-`[0.9.8]`-Eintrag + Tag `v0.9.8`, DoD §7)
> — siehe [`parquet-productive-cut-a.md`](parquet-productive-cut-a.md).
> Object-Storage-Track bleibt Plan-Entwurf
> ([`object-storage-artifact-store.md`](../next/object-storage-artifact-store.md));
> BI-Demo ist unter `examples/bi-demo/` geliefert.
