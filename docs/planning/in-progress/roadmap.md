...
#### Aktueller Arbeitsstand 0.9.7 (2026-05-28)
...
| E Rest | MySQL-Sequence-Diff-Migration ✅ (2026-05-20, Sub-Slices A–E; Plan-Doc `docs/planning/done/ImpPlan-0.9.7-mysql-sequence-diff-migration.md`). MySQL-Sequence-Drift-Check ✅ (2026-05-20, Sub-Slices A–F + Review-Follow-ups 1–4; Live-DB-Probe-Adapter analog F.5 E.3; Plan-Doc `docs/planning/done/ImpPlan-0.9.7-mysql-sequence-drift-check.md`). Sequence-preserveCurrentValue ✅ (2026-05-21, Sub-Slices A–E: Foundations + PG/MySQL-Renderer in A, PG-Probe in B, MySQL-Probe in C, Pipeline-Integration + Planner-Emit + CLI in D, Schema-Doku + Closing in E; Plan-Doc `docs/planning/done/ImpPlan-0.9.7-sequence-preserve-current-value.md`). SQLite-Trigger-Reverse-Read ✅ (2026-05-22, Sub-Slices A–E: token-basierter `SqliteTriggerSqlParser` in A, SchemaReader-Routing + Löschen des Substring-Pfads in B, Round-Trip-Idempotenz in C, Live-DB-Integration in D, Closing in E; Plan-Doc `docs/planning/done/ImpPlan-0.9.7-sqlite-trigger-reverse-read.md`). SQLite-Sequence-Emulation Phase A (§11 Pre-Code-Klärungen) und Phase B.0/B.1 abgeschlossen 2026-05-27 (`DdlDialectContext`-Refactor `48c7f01c`; `SqliteNamedSequenceMode` + `--sqlite-named-sequences` CLI-Plumbing `84ba7ab7`; Plan-Doc `docs/planning/in-progress/sqlite-sequence-emulation-plan.md`). SQLite-Sequence Phase B.2 (Validator-Regeln) abgeschlossen 2026-05-28: SequenceDefinition-internal-Regeln `E125` (`25f59f73`) im `SchemaSequenceValidationRules` (Increment/Bereich, overflow-sicheres `isIncrementInRange` nach §3.6) und helper_table-PK-Gate `E059` (`09068f79`) über neuen `PreGenerationValidator`-Port plus `SqliteHelperTableSequenceValidator` und `SqlitePreGenerationValidator`-Bridge, verdrahtet symmetrisch in `SchemaGenerateRunner` und `ToolExportRunner`. SQLite-Sequence Phase B.3 (helper_table-DDL + `_bi`/`_ai`-Trigger-Paar inkl. CHECK-`IS NOT NULL`-Auto-Suppression nach §3.4) abgeschlossen 2026-05-29: `SqliteSequenceDdlSupport` (state-pattern analog MySQL) + `SqliteSequenceNaming` + `SqliteSequenceEmulationTemplates`; `dmg_sequences`-Tabelle + Seed-INSERTs; overflow-sicheres `_bi`/`_ai`-Trigger-Paar mit `exhausted`-Flag; W114 (cache metadata-only), W115 (lossy NULL), W117 (transaction-bound globalNote), W119 (NOT NULL- und CHECK-`IS NOT NULL`-Suppression), E056 (action_required Per-Sequence/Per-Column), E057 (WITHOUT ROWID + SequenceNextVal), E124 (Reserved-Name-Kollisionen) verdrahtet; `SqliteCapabilityDdlSupport` delegiert; golden masters `full-featured.sqlite{.sql,.pre-data.sql}` auf neue ManualActionRequired-Texte gezogen. Offen bleiben: SQLite-Sequence Phase B.4 (`SequenceCapability`-Defaults flippen), C/D/E; atomare Probe+setval unter Lock, CreateSequence-Pre-Probe-Gate Widening, Multi-Sequence-Atomicity | teilerledigt |
