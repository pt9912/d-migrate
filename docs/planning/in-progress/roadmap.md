...
| Coverage/QA | MySQL-`AlterColumnNullability` ist als bewusster Blocker umgesetzt; Artifact-Compatibility (`UNKNOWN_FORMAT_VERSION` / `HASH_MISMATCH` / Secret-Scrubbing) + Overlay-Compatibility seit G-Slices vollstaendig gepinnt. **§11 DoD Box (a) Positiv+Blocker pro Workstream** ist ueber 22 Workstreams audit-sweep abgehakt (2026-05-19, siehe `diffresult-migration-plan-2.md` §11 DoD-Tabelle). Verbleibend: §11 DoD Box (b) Report-/Exit-Code-Tests + Box (c) Rollback-Tests je Workstream. | teilerledigt |
