...
- `README.md` existiert als Einstieg und verweist bereits auf Docker, Build und
  Dokumentation.
- `docs/guide.md` deckt heute `schema validate`, `schema generate`,
  `data export` und `data import` ab.
- `docs/cli-spec.md` existiert als Referenz, enthaelt aber noch veraltete
  Statusmarker:
  - `schema compare` steht noch als "geplant: 0.5.0"
  - `data import` steht noch als "geplant: 0.4.0", obwohl der Command laengst
    umgesetzt ist
- `docs/releasing.md` beschreibt heute Branching, Tagging, GHCR-Image und
  `gh release create`, aber noch nicht:
  - ZIP/TAR/Fat-JAR-Assets
  - SHA256-Datei
  - Workflow-Artefakte fuer Release-Assets
  - Homebrew-Formula-Pflege
- `docs/architecture.md` Abschnitt 5.3 beschreibt Distribution aktuell zu breit:
...
- Doku-Konsolidierung fuer:
  ...
  - `docs/cli-spec.md`
  - `docs/releasing.md`
  - `docs/architecture.md` Abschnitt Distribution
- optionaler Konsistenzabgleich in `docs/roadmap.md`, falls Statusmarker oder
...
- das eigentliche `gh release create` bzw. `gh release upload` bleibt ein
  expliziter Maintainer-Schritt in `docs/releasing.md`
...
| `docs/guide.md` | Compare-Quick-Start, Installations-/Nutzungsfluss fuer den MVP |
| `docs/releasing.md` | Release-Artefakte, Workflow-Artefakte, `gh release`-Schritte, Formula-Pflege |
| `docs/architecture.md` | Distribution fuer 0.5.0 ehrlich schaerfen |
| `docs/cli-spec.md` | Statusmarker und Referenzkonsistenz fuer implementierte Commands |

Optional, nur wenn fuer Konsistenz noetig:

| Datei | Aenderung |
|---|---|
| `docs/roadmap.md` | sprachliche Konsistenz rund um 0.5.0-Doku-/Build-Deliverables |
...
### 6.4 `docs/releasing.md`

`docs/releasing.md` ist der zentrale Maintainer-Runbook-Pfad fuer 0.5.0 und
...
- das gruene Tag-Artefakt `release-assets` wird in `docs/releasing.md` als
...
- `docs/releasing.md` nennt den exakten Formula-Pfad
- `docs/releasing.md` beschreibt den URL-/SHA-Bump
- die Formula basiert auf demselben Release-Dateinamen, den der Workflow und
  der Release-Ordner nutzen
- `docs/releasing.md` trennt klar zwischen lokalem Asset-Preflight und finalem
...
- `README.md`, `docs/guide.md`, `docs/releasing.md` und `docs/cli-spec.md`
  beschreiben den implementierten 0.5.0-Stand ohne veraltete
  "geplant"-Markierungen fuer bereits vorhandene Commands.
- `docs/architecture.md` Abschnitt Distribution beschreibt fuer 0.5.0 ehrlich
...
- `docs/releasing.md` beschreibt, wie genau diese Artefakte lokal oder aus dem
