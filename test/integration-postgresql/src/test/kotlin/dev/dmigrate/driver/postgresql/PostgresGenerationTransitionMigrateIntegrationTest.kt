...
class PostgresGenerationTransitionMigrateIntegrationTest : FunSpec({
    ...
    beforeSpec {
        ...
        pool.borrow().asJdbc().use { conn ->
            conn.createStatement().use { stmt ->
                ...
                stmt.execute("CREATE TABLE legacy_counters (id int NOT NULL UNIQUE, val int)")
                stmt.execute("INSERT INTO legacy_counters (id, val) VALUES (1, 10), (2, 20), (5, 50)")
            }
        }
    }
    ...
    /**
     * `column-generation-diff-unmapped.md`: eine gewoehnliche Spalte mit
     * Bestand (`1, 2, 5`) wird Identity. Ohne Sequenz-Nachziehung wuerde die
     * neue Sequenz bei 1 beginnen und der naechste `INSERT` am
     * UNIQUE-Constraint scheitern (`duplicate key value violates unique
     * constraint`, live gemessen gegen 18.6) — hier der Vollpfad, der genau
     * das verhindert.
     */
    test("an ordinary column becomes identity, and the sequence catches up past the existing max") {
        val want = desiredWith("legacy_counters", "id", ColumnGeneration.Identity(IdentityMode.BY_DEFAULT))

        val (exit, executed, errors) = migrate(want, changedTable = "legacy_counters")

        withClue(errors) { exit shouldBe 0 }
        withClue(executed.joinToString(" | ")) {
            executed.size shouldBe 3
            executed[0] shouldContain "ALTER COLUMN \"id\" SET NOT NULL"
            executed[1] shouldContain "ADD GENERATED BY DEFAULT AS IDENTITY"
            executed[2] shouldContain "setval(pg_get_serial_sequence"
        }

        val generation = liveSchema().tables.getValue("legacy_counters").columns.getValue("id").generation
        (generation as ColumnGeneration.Identity).mode shouldBe IdentityMode.BY_DEFAULT

        pool.borrow().asJdbc().use { conn ->
            conn.createStatement().use { it.execute("INSERT INTO legacy_counters (val) VALUES (100)") }
            conn.createStatement().use { stmt ->
                stmt.executeQuery("SELECT id FROM legacy_counters WHERE val = 100").use { rs ->
                    rs.next() shouldBe true
