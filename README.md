...
📖 **Documentation** (German): [Anwenderhandbuch](docs/user/anwenderhandbuch.md)
(task-oriented user guide) · [Administrationshandbuch](docs/user/administrationshandbuch.md)
(deployment/ops) · [CLI spec](spec/cli-spec.md) · [MCP server spec](spec/mcp-server.md)
— full index: [docs/user/README.md](docs/user/README.md). See
["Documentation"](#documentation) below for the complete list.
...
- **Schema operations**: `validate`, `generate`, `compare`,
  `reverse`, `migrate`, `rollback` for every dialect listed under
  [Supported databases](#supported-databases) — file/file, file/db,
  db/db.
...
- **Current stable** · **1.7.1** (2026-09-16) — what `:latest`,
  ...
  severity — additive to the wire contract, and the call stays a success
  (`isError=false`), because a failed call would replace the payload and lose
  the very findings and DDL it is about. The container image runs as
...
Published releases ship ZIP, TAR, a fat JAR and **native binaries**
that need no Java, on the
