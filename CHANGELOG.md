...
- **`AUTO_INCREMENT` als `serial` oder als `identity` lesen.** MySQL traegt
  ...
  SQL-Standard-IDENTITY gemeint war; der Reverse las sie immer als `serial`
  (`legacy_serial_syntax`, PostgreSQL erzeugt `BIGSERIAL`), und `schema compare`
  meldete jede PostgreSQL-IDENTITY-Spalte gegen sie als geaendert.
  `schema reverse --mysql-autoincrement-syntax identity` bzw.
  `reverse.mysql.autoincrement_syntax: identity` in `.d-migrate.yaml` liest sie
  ohne das Flag (PostgreSQL erzeugt dann `GENERATED BY DEFAULT AS IDENTITY`)
...
- **Der Sequenzname einer Identity-Spalte ist in `schema compare` kein Fund
  ...
  `sequenceName=public.customer_id_seq` gegen `null`. `legacy_serial_syntax`
  bleibt ein Unterschied; ob MySQL es setzt, entscheidet die neue
  Reverse-Praeferenz (unter „Added"). Der Modus bleibt ein Unterschied — auch
  gegen SQL Server (kein `BY DEFAULT`, `W140`), MySQL und SQLite, die nur ihre
  eine Autowert-Form kennen —, `schema migrate` und der Fingerabdruck sind
  unberuehrt.
...
- **`ddl.include_comments: false`** lässt `schema generate` ohne den
  `-- Generated by d-migrate …`-Header laufen, für alle fünf Dialekte
  (Default `true`, heutiges Verhalten unverändert). Kein CLI-Flag — der
...
- **Die Erzeugungsart einer Spalte lässt sich ändern, wo der Server es
  wirklich kann.** Bisher endete jede solche Änderung in einer Meldung; jetzt
  wird ausgeführt, was sich sicher ausführen lässt:

  | Übergang | PostgreSQL | MySQL | SQLite | Oracle |
  | --- | --- | --- | --- | --- |
  | berechnet → gewöhnlich | `DROP EXPRESSION` | — | Tabellen-Neubau | — |
  | Identity-Modus | `SET GENERATED …` | — | — | `MODIFY … AS IDENTITY` |
  | Identity entfernen | `DROP IDENTITY` | `MODIFY COLUMN` | — | `MODIFY … DROP IDENTITY` |
  | Identity hinzufügen | — | `MODIFY … AUTO_INCREMENT` | — | — |

  Jede Zelle ist **durch den vollen Migrationspfad** belegt, nicht nur als
  abgeschickte Anweisung — und das war nötig: Oracles `MODIFY` auf einer
  virtuellen Spalte wird vom Server **angenommen** und ändert nichts. Erst der
  Post-Compare zeigte, dass die Spalte weiterhin virtuell war. Der Übergang
  bleibt dort deshalb abgelehnt.

  Zwei Zusagen, die aus der Messung folgen: **PostgreSQL fügt keine Identity
  hinzu**, obwohl die Anweisung durchläuft — die neue Sequenz beginnt bei 1 und
  der nächste `INSERT` kollidiert mit dem Bestand; MySQL setzt den Zähler von
  selbst über den Bestand und darf es deshalb. Und **Oracle nimmt der Spalte
  mit der Identity ihr implizites `NOT NULL`** — der Renderer zieht es nach,
  sonst endete der Lauf in Drift.
...
- **Eine Typänderung auf MySQL nimmt der Spalte nicht mehr ihre übrigen
  Eigenschaften.** `MODIFY COLUMN` ersetzt die **ganze** Deklaration; gerendert
  wurden nur Name und Typ — mit der Begründung, die Operation ändere ja auch nur
  den Typ. Das schützt die Spalte nicht. Live gegen 9.7.2 gemessen fielen dabei
  still weg: `NOT NULL`, der `DEFAULT` und der `AUTO_INCREMENT`. Und das war
  kein bloß fehlgeschlagener Lauf — der Post-Compare meldete die Drift,
  **nachdem** die Anweisung angewandt war.

- **Ein MySQL-String-Default wird nicht mehr als Funktionsaufruf gelesen.**
  MySQL gibt `DEFAULT 'x'` in `COLUMN_DEFAULT` als `x` zurück — ohne
  Anführungszeichen. Der Leser fiel durch alle Zweige und machte daraus einen
  `FunctionCall`, gerendert als `DEFAULT x()`. Der Fall, der es zur
  Korrektheitsfrage macht, ist `DEFAULT 'UPPER(a)'`: ein Zeichenketten-Literal,
  das wie ein Aufruf aussieht und im Ziel **ausgeführt** würde.

  Die Unterscheidung liefert MySQL mit, in `EXTRA` (`DEFAULT_GENERATED` ⇔
  Ausdruck) — dieselbe Spalte, die schon die berechneten Spalten einordnet.
  Welche Art von Literal es ist, sagt jetzt der Spaltentyp: `'7'` in einer
  Textspalte bleibt eine Zeichenkette.

- **Ein Autowert-Schlüssel lässt sich auf MySQL von `identifier` auf
  `biginteger` verbreitern.** Genau die Migration, die fällig wird, wenn der
  INT-Bereich ausgeht — sie blockte, weil der Cast nicht in der Liste der
  sicheren stand, obwohl MySQL `MODIFY COLUMN id BIGINT NOT NULL AUTO_INCREMENT`
  annimmt. Die Gegenrichtung bleibt blockiert: sie verengt.

- **Eine PostgreSQL-Identity-Spalte außerhalb des Primärschlüssels verliert
  ihre Identity nicht mehr.** Ein `integer GENERATED BY DEFAULT AS IDENTITY`
  ohne Schlüssel kam als gewöhnliches `integer` zurück — weder im Typ noch in
  `generation` noch im Default stand die Identity, und ein `schema generate`
  daraus erzeugte eine Spalte ohne Generator. Zwei Zweige des Typ-Mappers
  fingen sie nicht: der eine nur `bigint`, der andere nur den Primärschlüssel.
...
- **Oracle: `schema migrate` macht eine bestehende Spalte zur
  ...
  — anlegen, kopieren, alte löschen, umbenennen — und stellt Primärschlüssel,
  Constraints, Indizes und die eingehenden Fremdschlüssel wieder her, die
  `DROP TABLE … CASCADE CONSTRAINTS` mit abräumt. Die vorhandenen Schlüssel
  überleben (die Zwischentabelle nimmt sie als `GENERATED BY DEFAULT` an), und
  `START WITH LIMIT VALUE` setzt den Zähler danach über den größten davon.
  Gegen ein echtes Oracle 23 abgenommen. Der Neubau ist destruktiv und
...
- **Post-Compare meldete Drift auf einer korrekt angewendeten
  ...
  der Reverse liefert die zweite, ein handgeschriebenes Soll meist die erste.
  Der Vergleich faltete beide bereits zusammen, der Fingerabdruck nicht — ein
  `migrate --execute` gegen Oracle, SQL Server, MySQL oder SQLite endete
  deshalb mit Exit 5, obwohl die Spalte genau wie gewünscht angewendet worden
  war. PostgreSQL bleibt ausgenommen: dort sind `SERIAL` und
  `GENERATED … AS IDENTITY` zwei verschiedene Dinge.
...
- **Ein Enum gilt im Post-Compare unabhaengig von seiner Darstellung**
  ...
  CHECK daneben — dieselbe Aussage, bisher aber zwei verschiedene
  Fingerprints. Damit meldete jede Migration mit Enum-Spalte auf einem
  Dialekt ohne Enum-Typ Drift, auch wenn sie fehlerfrei lief.
  `schema compare` bleibt unveraendert streng.

  **Fingerprint-Algorithmus `schema-fingerprint-v7` → `v8`:** aeltere
  Rollback-Artefakte und Overlays sind nicht mehr vergleichbar und werden
  mit `ROLLBACK_FINGERPRINT_ALGORITHM_MISMATCH` abgelehnt statt still
  falsch verglichen. Neu erzeugen.
