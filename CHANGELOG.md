...
- **Gespeicherte Zugangsdaten werden beim Verbinden herangezogen** ([`LN-049`](spec/lastenheft-d-migrate.md#ln-049)
  ...
  Verdrahtet für `data export`/`import`/`transfer` (Quelle **und** Ziel, eine geteilte Master-Secret-Session),
  `schema compare` und `schema reverse` (Store-Key = der `--source`/`--target`-Connection-Name). Offen:
  `data profile` (opaker `poolFactory`) und `schema migrate`/`rollback` (deren Preflight-Probes brauchen eine
  gemeinsam durchgereichte Master-Secret-Session) — beides folgt mit der uniformen Namens-Durchreichung.
