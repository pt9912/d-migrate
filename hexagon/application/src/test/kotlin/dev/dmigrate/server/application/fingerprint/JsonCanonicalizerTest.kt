...
class JsonCanonicalizerTest : FunSpec({
    ...
    test("primitives serialize without whitespace") {
        canonicalizer.canonicalize(JsonValue.Null) shouldBe "null"
        canonicalizer.canonicalize(JsonValue.Bool(true)) shouldBe "true"
        canonicalizer.canonicalize(JsonValue.Bool(false)) shouldBe "false"
        canonicalizer.canonicalize(JsonValue.Num(0)) shouldBe "0"
        canonicalizer.canonicalize(JsonValue.Num(-42)) shouldBe "-42"
        canonicalizer.canonicalize(JsonValue.Num(Long.MAX_VALUE)) shouldBe Long.MAX_VALUE.toString()
        canonicalizer.canonicalize(JsonValue.Str("abc")) shouldBe "\"abc\""
    }
    ...
    test("object keys are sorted by UTF-16 code unit") {
        ...
        canonicalizer.canonicalize(obj) shouldBe "{\"a\":2,\"m\":3,\"z\":1}"
    }

    test("array order is preserved") {
        val arr = JsonValue.Arr(listOf(JsonValue.Num(3), JsonValue.Num(1), JsonValue.Num(2)))
        canonicalizer.canonicalize(arr) shouldBe "[3,1,2]"
    }

    test("strings escape only quote, backslash and controls below 0x20") {
        canonicalizer.canonicalize(JsonValue.Str("a\"b")) shouldBe "\"a\\\"b\""
        canonicalizer.canonicalize(JsonValue.Str("a\\b")) shouldBe "\"a\\\\b\""
        canonicalizer.canonicalize(JsonValue.Str("a\nb")) shouldBe "\"a\\u000ab\""
        canonicalizer.canonicalize(JsonValue.Str("a\tb")) shouldBe "\"a\\u0009b\""
        canonicalizer.canonicalize(JsonValue.Str("a\u0000b")) shouldBe "\"a\\u0000b\""
    }

    test("non-control extended characters stay literal (not escaped)") {
        canonicalizer.canonicalize(JsonValue.Str("café")) shouldBe "\"café\""
        canonicalizer.canonicalize(JsonValue.Str("über")) shouldBe "\"über\""
    }

    test("string values are NFC-normalized before output") {
        val decomposed = "cafe\u0301"
        val composed = "café"
        canonicalizer.canonicalize(JsonValue.Str(decomposed)) shouldBe
            canonicalizer.canonicalize(JsonValue.Str(composed))
    }
    ...
    test("object keys are NFC-normalized before sorting") {
        ...
        canonicalizer.canonicalize(a) shouldBe canonicalizer.canonicalize(b)
    }
    ...
    test("nested structures canonicalize recursively") {
        ...
        canonicalizer.canonicalize(nested) shouldBe
            "{\"outer\":{\"a\":true,\"z\":[2,1]}}"
    }

    test("empty object and array") {
        canonicalizer.canonicalize(JsonValue.Obj.EMPTY) shouldBe "{}"
        canonicalizer.canonicalize(JsonValue.Arr(emptyList())) shouldBe "[]"
    }

    test("null is preserved as a value") {
        val obj = JsonValue.obj("missing" to JsonValue.Null, "present" to JsonValue.Num(1))
        canonicalizer.canonicalize(obj) shouldBe "{\"missing\":null,\"present\":1}"
    }
    ...
    test("UTF-16 code unit ordering separates ascii and BMP characters") {
        ...
        canonicalizer.canonicalize(obj) shouldBe "{\"z\":2,\"ä\":1}"
    }
