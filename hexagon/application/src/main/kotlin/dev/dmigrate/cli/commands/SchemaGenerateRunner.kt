...
data class SchemaGenerateRequest(
    ...
    /** `ddl.include_comments` — ob der `-- Generated by …`-Header entsteht. Default `true`. */
    val includeComments: Boolean = true,
    /**
     * `ddl.postgresql.default_schema` — das Schema, in dem ein PostgreSQL-Ziel
     * die erzeugten Objekte anlegt. `null`: unqualifiziert, heutiges Verhalten.
     * Wirkungslos fuer jeden anderen Dialekt.
     */
    ...
    private fun validateAndResolveOptions(request: SchemaGenerateRequest): Preflight {
        ...
        val dialectContext: DdlDialectContext = when (dialect) {
            ...
            DatabaseDialect.POSTGRESQL ->
                DdlDialectContext.Postgres(
                    serverVersion = targetVersion.value as? PostgresServerVersion,
                    defaultSchema = request.postgresqlDefaultSchema,
                )
