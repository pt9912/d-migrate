...
data class DdlGenerationOptions(
    ...
    /**
     * `ddl.include_comments`: ob `AbstractDdlGenerator.generateHeader()` den
     * `-- Generated by d-migrate ...`-Kommentarblock an den Anfang der
     * erzeugten DDL setzt. Default `true` (heutiges Verhalten unveraendert).
     *
     * Kein CLI-Flag — der Header beschreibt das Ziel, nicht den einzelnen
     * Aufruf (dieselbe Ebene wie `MysqlTableOptions`/`mysql.engine`), und ein
     * Boolean-CLI-Flag haette hier keine natuerliche Drei-Wege-Praezedenz
     * (anders als die String-wertigen Schluessel mit `null` = "nicht
     * gesetzt"). Siehe `docs/planning/done/ddl-comment-rendering.md`.
     */
